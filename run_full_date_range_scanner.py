#!/usr/bin/env python3
"""
Run the backside B scanner from 1/1/24 to 11/1/25 (full date range)
"""
...
def create_full_date_range_scanner():
    """Create a version with full date range 1/1/24 to 11/1/25"""
    ...
    # Replace the smart filtering with comprehensive symbol list
    ...
    # Modify date range to full 1/1/24 to 11/1/25
    content = content.replace('fetch_start = "2020-01-01"', 'fetch_start = "2024-01-01"')
    content = content.replace('fetch_end   = datetime.today().strftime("%Y-%m-%d")', 'fetch_end   = "2025-11-01"')

    # Write to temp file
    temp_path = Path("/Users/michaeldurante/ai dev/ce-hub/temp_full_date_range_scanner.py")
    with open(temp_path, 'w') as f:
        f.write(content)

    return temp_path

def run_full_date_range():
    """Run the scanner with full date range"""
    print("🏃 RUNNING BACKSIDE B SCANNER - FULL DATE RANGE")
    print("=" * 80)
    print("📅 Date Range: 1/1/24 - 11/1/25 (Nearly 2 full years)")
    print("🎯 Full Parameters: $8.00 min price, $30M min ADV, 15M min D-1 volume")
    print("📊 Symbol List: 300+ comprehensive tickers (including BABA)")
    print("🚫 Smart Filtering: COMPLETELY DISABLED")
    print("✅ Full Universe: ENABLED")
    print()

    # Create modified scanner
    temp_scanner = create_full_date_range_scanner()

    try:
        # Run the scanner
        result = subprocess.run([
            "python3", str(temp_scanner)
        ], capture_output=True, text=True, timeout=1200)  # 20 minute timeout for full date range
        ...
        print("\n⏰ SCANNER TIMEOUT - taking too long (large date range)")
        return False
...
def main():
    """Main runner"""
    print("🧪 BACKSIDE B SCANNER - FULL DATE RANGE TEST")
    print("Running from 1/1/24 to 11/1/25 for comprehensive backside pattern analysis")
    print("=" * 90)

    success = run_full_date_range()

    print("\n" + "=" * 90)
    if success:
        print("🎉 FULL DATE RANGE SCAN COMPLETED")
        print("This should show the true annual frequency of backside B patterns")
        print("Expecting ~10-20 results total across nearly 2 years")
    else:
        print("❌ FULL DATE RANGE SCAN FAILED")
