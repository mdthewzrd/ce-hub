...
def check_high_lvl_filter_lc(df):
    ...
    # SINGLE SCANNER LOGIC: lc_frontside_d2_extended
    ...
    # Filter for single scanner
    df2 = df[df['lc_frontside_d2_extended'] == 1]
    return df2


def filter_lc_rows(df):
    # UPDATED FOR SINGLE SCANNER: lc_frontside_d2_extended
...
def get_min_price_lc(df):
    # SCANNER-SPECIFIC MIN PRICE: lc_frontside_d2_extended
    df['lc_frontside_d2_extended_min_price'] = round((df['c'] + df['d1_range']*.5), 2)

    # Single scanner minimum price calculation
...
async def main():
    ...
    df_lc.to_csv("lc_frontside_d2_extended_backtest.csv")
