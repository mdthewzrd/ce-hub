...
class CustomScannerManager:
    ...
    def __init__(self):
        ...
        self.scanner_metadata = {
            'backside_para_b': {
                ...
                'return_columns': ['symbol', 'date', 'close', 'trigger_price', 'atr', 'volume', 'rsi']
            },
            'half_a_plus': {
    ...
    async def run_backside_scanner(self, start_date: str, end_date: str, symbols: List[str] = None, params: Dict = None) -> Dict:
        ...
        try:
            ...
            scan_params = self.scanner_metadata['backside_para_b']['param_structure'].copy()
            if params:
            ...
            for symbol in all_symbols:
                try:
                    result = module.scan_symbol(symbol, start_date, end_date)
                    if not result.empty:
                        # Convert DataFrame to dict and add parameter info
                        symbol_results = result.to_dict('records')
                        for record in symbol_results:
                            record['scanner_type'] = 'backside_para_b'
