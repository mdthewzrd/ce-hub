...
// Disabled direct OpenRouter integration - now routing through API Gateway
// import { glmCodeFormatter } from './glmCodeFormatter';
...
export class TradingCodeFormatter {
  ...
  public async formatTradingCode(
    ...
    // Step 2: Try API Gateway AI formatting (centralized service)
    try {
      console.log('🚀 Attempting API Gateway AI formatting...');
      const gatewayResult = await this.formatThroughAPIGateway(code, options);
      ...
      console.warn('⚠️ API Gateway formatting failed, falling back to backend:', gatewayError);
    }
  ...
  /**
   * Format code through API Gateway (centralized AI service)
   */
  private async formatThroughAPIGateway(
    code: string,
    options: CodeFormattingOptions = {}
  ): Promise<FormattingResult> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`API Gateway request failed: ${response.status} ${response.statusText}`);
      }

      const apiResult = await response.json();
      console.log('  API Gateway formatting completed successfully!');

      // Extract the formatted code from backend response
      let formattedCode: string;
      if (typeof apiResult.formatted_code === 'string') {
        formattedCode = apiResult.formatted_code;
      } else if (typeof apiResult.data?.enhanced_code === 'string') {
        formattedCode = apiResult.data.enhanced_code;
      } else if (typeof apiResult.data === 'string') {
        formattedCode = apiResult.data;
      } else {
        formattedCode = code;
      }
      ...
      return {
        ...
        scannerType: 'api_gateway_enhanced',
        integrityVerified: true,
        originalSignature: code.substring(0, 100),
        formattedSignature: formattedCode.substring(0, 100),
        optimizations: [
          'API Gateway AI-powered formatting',
          'Centralized rate limiting',
          'DeepSeek paid model integration',
          'Request queuing',
          ...(apiResult.data.enhancements_applied || [])
        ],
        warnings: [],
        errors: apiResult.success ? [] : [apiResult.message || 'API Gateway formatting failed'],
        metadata: {
          originalLines: code.split('\n').length,
          formattedLines: formattedCode.split('\n').length,
          scannerType: 'api_gateway_enhanced',
          parameterCount: apiResult.data.original_metrics?.functions_found || 0,
          processingTime: `${apiResult.execution_time}s`,
          infrastructureEnhancements: [
            'API Gateway with rate limiting',
            'DeepSeek paid model (no rate limits)',
            'Request queuing and throttling',
            'Centralized AI processing',
            'Bulletproof parameter integrity'
          ]
        }
      };

    } catch (error) {
      console.error('❌ API Gateway formatting failed:', error);
      throw error;
    }
  }
