...
import { Brain, Upload, Play, TrendingUp, BarChart3, Settings, Search, Target, Filter, Clock, Database, MessageCircle, ChevronUp, ChevronDown, Trash2, RefreshCw, Save, Plus, X } from 'lucide-react';
...
import { AddScannerRequest } from '@/types/projectTypes';
...
export default function Home() {
  ...
  const handleRunScan = async () => {
    ...
    try {
      ...
      // First get the projects to find the most recent user project
      const projectsResponse = await fetch('http://localhost:5659/api/projects');
      const projects = await projectsResponse.json();

      // Find the most recent user-uploaded project (with formatted code)
      const userProject = projects.find((p: any) => p.tags && p.tags.includes('uploaded'));
      const projectId = userProject ? userProject.id : '3';

      console.log('  Using project ID:', projectId, userProject?.name || 'Default');

      const response = await fetch(`http://localhost:5659/api/projects/${projectId}/execute`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success && data.execution_id) {
          ...
          // Extract actual scan results array from backend response
          const actualScanResults = data.results?.top_matches || data.results?.all_results || [];

          // Convert backend scan results to expected format
          const convertedResults = actualScanResults.map((result: any) => ({
            ticker: result.symbol || 'UNKNOWN',
            date: result.timestamp || new Date().toISOString().split('T')[0],
            gapPercent: parseFloat(result.gap_percent) || 0,
            volume: result.volume || 0,
            score: result.confidence || 0,
            result: result.signal?.toLowerCase() === 'buy' ? 'win' : 'loss',
            pnl: result.pnl || '0.0%',
            execution_output: result.execution_output
          }));

          console.log('  Scan completed successfully, setting results:', convertedResults);
          setScanResults(convertedResults);
        } else {
          // Fallback to mock data if no execution_id
          console.log('❌ API response missing execution_id, using sample data');
...
if __name__ == "__main__":
  ...
  // Functions for "Add to Projects" functionality
  const handleAddToProject = () => {
    if (!formattedCode.trim()) {
      alert('No formatted code available to add to project');
      return;
    }

    // Generate scanner name from file name or use default
    const defaultScannerName = uploadedFileName
      ? uploadedFileName.replace(/\.(py|txt)$/, '')
      : `scanner_${new Date().toISOString().slice(0, 10)}`;

    setScannerName(defaultScannerName);
    setShowProjectSelection(true);
  };

  const handleAddScannerToProject = async () => {
    if (!selectedProjectId || !scannerName.trim() || !formattedCode.trim()) {
      alert('Please select a project and provide a scanner name');
      return;
    }
    ...
    try {
      console.log(`🚀 Adding scanner "${scannerName}" to project ${selectedProjectId}`);

      // Step 1: Save the formatted scanner to the system
      const saveResponse = await fetch('http://localhost:5659/api/format/save-scanner-to-system', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          parameters_count: parseScanParameters(formattedCode).filters?.length || 0,
          scanner_type: 'formatted_scanner',
          user_id: 'edge_dev_user',
          project_id: selectedProjectId // Link directly to project
        }),
      });

      if (!saveResponse.ok) {
        throw new Error(`Failed to save scanner: ${saveResponse.statusText}`);
      }

      const saveResult = await saveResponse.json();
      console.log('✅ Scanner saved to system:', saveResult);

      // Step 2: Add the scanner to the project
      const scannerRequest: AddScannerRequest = {
        scanner_id: saveResult.scanner_id || `${scannerName}_${Date.now()}`,
        scanner_file: saveResult.scanner_file || `${scannerName}.py`,
        enabled: true,
        weight: 1.0,
        order_index: 1
      };

      const addResponse = await fetch(`http://localhost:5659/api/projects/${selectedProjectId}/scanners`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(scannerRequest),
      });

      if (!addResponse.ok) {
        throw new Error(`Failed to add scanner to project: ${addResponse.statusText}`);
      }

      const addResult = await addResponse.json();
      console.log('✅ Scanner added to project:', addResult);

      // Step 3: Refresh projects to show the updated scanner count
      await loadProjects();
      ...
      alert(`✅ Scanner "${scannerName}" successfully added to project!`);

    } catch (error) {
      console.error('❌ Failed to add scanner to project:', error);
      alert(`Failed to add scanner to project: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute top-2 right-2 flex gap-2">
                        <button
                          ...
                          className="px-3 py-1 text-xs font-medium rounded transition-colors bg-green-600 text-white hover:bg-green-700"
                          title="Add this scanner to an existing project"
                        >
                          + Add to Project
                        </button>
                        <button
...
# Your code here...`
      ...
      {/* Project Selection Modal - Add to Projects */}
      {showProjectSelection && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="studio-surface border studio-border rounded-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="flex items-center justify-between p-6 border-b" style={{ borderColor: 'var(--studio-border)' }}>
              <div className="flex items-center gap-3">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Enter a name for this scanner"
                  className="studio-input w-full px-4 py-2"
                />
                <p className="text-xs mt-1" style={{ color: 'var(--studio-text-muted)' }}>
                  This name will be used to identify your scanner in the project.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {projects.length === 0 ? (
                    <div className="studio-surface border studio-border rounded-lg p-4 text-center">
                      <p className="text-sm" style={{ color: 'var(--studio-text-muted)' }}>
                    ...
                    projects.map((project) => (
                      <div
                        key={project.id}
                        className={`studio-surface border rounded-lg p-4 cursor-pointer transition-all ${
                          selectedProjectId === project.id
                            ? 'border-green-500 bg-green-500/10'
                            : 'studio-border hover:border-gray-500'
                        }`}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm" style={{ color: 'var(--studio-text-muted)' }}>
                              {project.project_data?.description || project.description || 'No description'}
                            </p>
                            <div className="flex items-center gap-4 mt-2 text-xs" style={{ color: 'var(--studio-text-muted)' }}>
                              <span>📊 {project.project_data?.scanner_count || 0} scanners</span>
                              <span>🏷️ {project.project_data?.aggregation_method || 'union'}</span>
                              <span>📅 Created {new Date(project.project_data?.created_at || project.created_at).toLocaleDateString()}</span>
                            </div>
                          </div>
                          <div className="w-4 h-4 rounded-full border-2 flex items-center justify-center"
                            style={{ borderColor: selectedProjectId === project.id ? '#10b981' : 'var(--studio-border)' }}>
                            {selectedProjectId === project.id && (
              ...
              {/* Formatted Code Summary */}
              {formattedCode && (
                <div>
                  <label className="block text-sm font-medium mb-2" style={{ color: 'var(--studio-text-secondary)' }}>
                    Scanner Summary
                  </label>
                  <div className="studio-surface border studio-border rounded-lg p-4">
                    {(() => {
                      const params = parseScanParameters(formattedCode);
                      return (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            ...
                            <div className="font-medium">{params.scanType || 'Unknown'}</div>
                          </div>
                          <div>
                            <span className="text-gray-400">Filters:</span>
                            <div className="font-medium">{params.filters?.length || 0}</div>
                          </div>
                          <div>
                            <span className="text-gray-400">Universe:</span>
                            <div className="font-medium">{params.tickerUniverse?.length || 0} tickers</div>
                          </div>
                          <div>
                            <span className="text-gray-400">Timeframe:</span>
                            <div className="font-medium">{params.timeframe || 'Daily'}</div>
                          </div>
                        </div>
                      );
                    })()}
              ...
              {/* Action Buttons */}
              <div className="flex gap-3 pt-4 border-t" style={{ borderColor: 'var(--studio-border)' }}>
                <button
                  onClick={() => {
                    setShowProjectSelection(false);
                    setSelectedProjectId('');
                ...
                <button
                  onClick={handleAddScannerToProject}
                  disabled={!selectedProjectId || !scannerName.trim() || isAddingScanner}
                  className="flex-1 px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isAddingScanner ? (
                    <>
