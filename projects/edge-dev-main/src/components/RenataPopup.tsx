...
import { Send, MessageCircle, Brain, X, ChevronUp, ChevronDown, Upload, Paperclip, TrendingUp, BarChart3, Zap, Search, Bell, Activity, Code, ChevronDown as ExpandIcon, ChevronUp as CollapseIcon, Plus, Play, FolderOpen } from 'lucide-react'
...
const CollapsibleCodeBlock: React.FC<CollapsibleCodeBlockProps> = ({ children, className = '' }) => {
  ...
  // Always show only first 3 lines in preview, with expand option
...
interface ActionButtonsProps {
  onFormatCode?: () => void
  onAddToProject?: (scannerName?: string) => void
  scannerName?: string
  showFormatButton?: boolean
  showAddToProjectButton?: boolean
}
...
const ActionButtons: React.FC<ActionButtonsProps> = ({
  ...
  if (!showFormatButton && !showAddToProjectButton) return null
  ...
  return (
    <div style={{
      ...
      {showAddToProjectButton && (
        <button
          onClick={() => onAddToProject?.(scannerName)}
          style={{
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
...
const RenataPopup: React.FC<RenataPopupProps> = ({ isOpen, onToggle }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      content: "**Renata - AI Trading Assistant**\n\nHello! I can help you format, optimize, and integrate trading scanner code.\n\nUpload your Python code or describe what you need!",
      type: 'assistant',
      timestamp: new Date()
    }
  ])
  ...
  // NEW: State to preserve formatted code between workflow steps
  const [formattedCode, setFormattedCode] = useState<string>('')
  const [lastFormattedCode, setLastFormattedCode] = useState<string>('')
  ...
  const addMessage = (message: Omit<ChatMessage, 'id' | 'timestamp'>) => {
    ...
    // Check if this message contains formatted code and extract it
    if (message.type === 'assistant' &&
        (message.content.includes('Code Formatting Complete') ||
         message.content.includes('📝 **Code Formatting Complete!**'))) {
      extractFormattedCode(message.content)
    }
  }

  // Extract formatted code from AI response
  const extractFormattedCode = (content: string) => {
    // Look for code blocks in the response
    const codeBlockMatch = content.match(/```(?:python)?\s*([\s\S]*?)\s*```/i)
    if (codeBlockMatch) {
      const extractedCode = codeBlockMatch[1]
      setFormattedCode(extractedCode)
      setLastFormattedCode(extractedCode)
      console.log('✅ Formatted code captured and stored:', extractedCode.length, 'characters')
    } else {
      console.warn('⚠️ No code block found in formatting response')
    }
  }

  // Action button handlers
  const handleFormatCode = () => {
    setCurrentInput('format code')
    // Automatically send the format request
    setTimeout(() => {
      handleSendMessage()
    }, 100)
  }

  const handleAddToProject = async (scannerName?: string) => {
    if (!scannerName) return

    // Validate that we have formatted code available
    if (!formattedCode && !lastFormattedCode) {
      console.warn('⚠️ No formatted code available for project integration')
      addMessage({
        type: 'assistant',
        content: '❌ No formatted code available. Please format your code first before adding to a project.',
    ...
    addMessage({
      type: 'user',
      content: `Add ${scannerName} to edge.dev project`
    })

    try {
      // Send direct request with proper context for project integration
      const response = await fetch('/api/renata/chat', {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            formattedCode: formattedCode || lastFormattedCode,
            page: 'renata-popup',
            timestamp: new Date().toISOString()
          }
        })
      ...
      if (response.ok) {
        ...
        addMessage({
          type: 'assistant',
          content: `❌ Failed to add "${scannerName}" to project. Please try again.`,
          timestamp: new Date().toISOString()
        })
      }
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    setIsLoading(true)
    setProgressStep(0)
    setProgressStatus('Initializing request...')

    // Start real progress tracking with SSE
    const eventSource = new EventSource('/api/renata/progress')
    ...
    eventSource.onerror = () => {
      // Fallback to generic messages if SSE fails
                  ...
                  <>
                    <ReactMarkdown
                      components={{
                        ...
                        p: ({children}) => <p style={{margin: '0 0 0.75rem 0', color: '#FFD700', lineHeight: '1.6', wordBreak: 'break-word', overflowWrap: 'break-word', padding: '0.25rem 0'}}>{children}</p>,
                    ...
                    {message.type === 'assistant' &&
                     (message.content.includes('Code Formatting Complete') || message.content.includes('📝 **Code Formatting Complete!**')) &&
                     (formattedCode || lastFormattedCode) && (
                      <ActionButtons
                        showAddToProjectButton={true}
                        onAddToProject={handleAddToProject}
                        scannerName={message.content.match(/Scanner Name:\s*\*\*([^*]+)\*\*/)?.[1] || 'Backside Para B Copy'}
                      />
                    )}
                  </>
                ) : (
