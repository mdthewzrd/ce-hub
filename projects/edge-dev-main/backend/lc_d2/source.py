...
def check_next_day_valid_lc(df):
    ...
    # FIXED: Use only the 3 patterns that are actually created (line 896)
...
def check_lc_pm_liquidity(df):
    # FIXED: Use only the 3 patterns that are actually created (line 896)
    df.loc[
        ((df['lc_frontside_d3_extended_1'] == 1) | (df['lc_frontside_d2_extended'] == 1) | (df['lc_frontside_d2_extended_1'] == 1)) &
        (df['valid_pm_liq'] != 1),
...
if __name__ == "__main__":
    ...
    END_DATE = '2025-12-31'  # Scan all 2025  
