...
class GuaranteedAIScannerService:
    ...
    async def split_scanner_intelligent(self, code: str, filename: str) -> Dict[str, Any]:
        ...
        SMART DETECTION: Distinguishes between single scanner scripts and multi-function scanners
        """
        logger.info(f"🎯 Real AI Scanner Analysis for {filename}")

        try:
            # PHASE 1: Smart Detection - Determine if this is a single scanner script or multi-function scanner
            scanner_type = self._detect_scanner_type(code, filename)
            logger.info(f"📊 Detected scanner type: {scanner_type}")

            if scanner_type == "single_script":
                logger.info(f"🎯 Single scanner script detected - creating 1 scanner from entire file")
                return self._create_single_scanner_from_script(code, filename)

            # PHASE 2A: Always attempt real AI analysis first for multi-function files
...
'''
    ...
    def _detect_scanner_type(self, code: str, filename: str) -> str:
        """
        Smart detection: Determine if this is a single scanner script or multi-function scanner
        """
        try:
            # Check for indicators of single scanner script
            indicators_single_script = [
                # Has main execution block
                'if __name__ == "__main__":' in code,
                # Has print statements for results (common in single scripts)
                'print(' in code and 'to_string(' in code,
                # Has data fetching patterns
                'fetch_daily(' in code or 'polygon.io' in code or 'SYMBOLS = [' in code,
                # Has parameter dictionary at top level
                'P = {' in code,
                # Has single scan function without .astype(int) return
                'def scan_symbol(' in code and '.astype(int)' not in code,
                # File is reasonably sized (not a huge multi-scanner file)
                len(code) < 20000,
                # Doesn't have multiple .astype(int) patterns (indicates multi-function scanner)
                code.count('.astype(int)') <= 1
            ]

            # Count indicators
            single_script_score = sum(indicators_single_script)

            # Check for multi-function indicators
            ast_functions = self._extract_scanner_functions_ast(code)
            multi_function_score = len(ast_functions)

            logger.info(f"🔍 Detection scores: single_script={single_script_score}, multi_function={multi_function_score}")

            # Decision logic
            if single_script_score >= 3 and multi_function_score == 0:
                return "single_script"
            elif multi_function_score >= 2:
                return "multi_function"
            elif single_script_score >= 2:
                return "single_script"
            else:
                # Default to multi_function (existing behavior)
                return "multi_function"

        except Exception as e:
            logger.error(f"❌ Error detecting scanner type: {str(e)}")
            # Default to multi_function (existing behavior)
            return "multi_function"

    def _create_single_scanner_from_script(self, code: str, filename: str) -> Dict[str, Any]:
        """
        Create a single scanner from an entire script file
        This handles files like 'backside para b copy.py' that are complete scanner scripts
        """
        try:
            logger.info(f"🎯 Creating single scanner from script: {filename}")

            # Extract scanner name from filename
            scanner_name_base = filename.replace('.py', '').replace(' ', '_').replace('-', '_').replace('copy', '').strip('_')
            scanner_name = f"{scanner_name_base}_AI_Generated"

            # Extract description from file comments or create default
            description = self._extract_description_from_code(code, filename)

            # Extract parameters from the entire script
            parameters = self._extract_parameters_from_script(code)

            # Create scanner object
            scanner = {
                'scanner_name': scanner_name,
                'description': description,
                'scanner_code': code,  # Use the entire original code
                'formatted_code': code,  # Use the entire original code
                'parameters': parameters,
                'complexity': min(len(parameters), 10),
                'dependencies': []
            }

            logger.info(f"✅ Created single scanner: {scanner_name} with {len(parameters)} parameters")
            ...
            return {
                ...
                'total_complexity': scanner['complexity'],
                'model_used': 'Single_Script_Detector',
                'method': 'Complete_Script_Analysis',
                'timestamp': datetime.now().isoformat()
            }

        except Exception as e:
            logger.error(f"❌ Error creating single scanner from script: {str(e)}")
            # Fallback to guaranteed
            return self._create_guaranteed_success()

    def _extract_description_from_code(self, code: str, filename: str) -> str:
        """Extract description from code comments or create default"""
        try:
            ...
            for line in lines[:10]:  # Check first 10 lines
                ...
                if line.startswith('#'):
                    ...
                    if comment_text and len(comment_text) > 5:  # Skip short comments
            ...
            if description_lines:
                description = ' '.join(description_lines[:2])  # Use first 2 comment lines
                return description[:100]  # Limit length

            # Fallback description based on filename
            base_name = filename.replace('.py', '').replace('_', ' ').replace('-', ' ').title()
            return f"Trading scanner: {base_name}"

        except Exception:
            # Ultimate fallback
            return f"Trading scanner from {filename}"

    def _extract_parameters_from_script(self, code: str) -> List[Dict[str, Any]]:
        """
        Extract parameters from a complete scanner script
        Looks for common parameter patterns in trading scanners
        """
        try:
            parameters = []

            # Extract parameters from P dictionary (common pattern)
            import re
            p_dict_pattern = r'P\s*=\s*\{([^}]+)\}'
            p_dict_match = re.search(p_dict_pattern, code, re.DOTALL)
            if p_dict_match:
                p_dict_content = p_dict_match.group(1)
                # Extract key-value pairs from P dictionary
                param_pattern = r'"([^"]+)"\s*:\s*([^,}]+)'
                for match in re.finditer(param_pattern, p_dict_content):
                    ...
                    try:
                        ...
                        if param_value_str.startswith('"') and param_value_str.endswith('"'):
                            ...
                            param_type = 'float' if isinstance(param_value, float) else 'integer'

                        # Determine category and importance
                        category = self._categorize_parameter(param_name)
                        importance = self._determine_importance(param_name, category)
                        ...
                        parameters.append({
                            ...
                            'type': param_type,
                            'category': category,
                            'description': f'Parameter: {param_name.replace("_", " ").title()}',
            ...
            # If no P dictionary found, look for other patterns
            if not parameters:
                # Look for variable assignments at top level
                var_pattern = r'^([a-zA-Z_]\w*)\s*=\s*(\d+\.?\d*)\s*$'
                for match in re.finditer(var_pattern, code, re.MULTILINE):
                    var_name = match.group(1)
                    var_value_str = match.group(2)

                    # Skip common non-parameter variables
                    skip_vars = {'MAX_WORKERS', 'API_KEY', 'BASE_URL', 'session'}
                    if var_name in skip_vars:
                        continue

                    try:
                        var_value = float(var_value_str) if '.' in var_value_str else int(var_value_str)
                        category = self._categorize_parameter(var_name)
                        importance = self._determine_importance(var_name, category)

                        parameters.append({
                            'name': var_name,
                            'current_value': var_value,
                            'type': 'float' if isinstance(var_value, float) else 'integer',
                            'category': category,
                            'description': f'Configuration parameter: {var_name.replace("_", " ").title()}',
                            'importance': importance
                        })
                    except Exception:
                        continue

            # If still no parameters, create basic defaults
            if not parameters:
                parameters = [
                    {'name': 'price_min', 'current_value': 8.0, 'type': 'float', 'category': 'price', 'description': 'Minimum price threshold', 'importance': 'high'},
                    {'name': 'volume_min', 'current_value': 1000000, 'type': 'integer', 'category': 'volume', 'description': 'Minimum volume requirement', 'importance': 'high'},
                    {'name': 'atr_multiplier', 'current_value': 1.0, 'type': 'float', 'category': 'technical', 'description': 'ATR multiplier for signals', 'importance': 'medium'}
                ]

            # Limit to 15 parameters and remove duplicates
            seen_names = set()
            unique_params = []
            ...
            logger.error(f"❌ Error extracting parameters from script: {str(e)}")
            # Return basic parameters as fallback
            return [
                {'name': 'threshold', 'current_value': 1.0, 'type': 'float', 'category': 'technical', 'description': 'Default threshold', 'importance': 'high'}
            ]
    ...
    def _categorize_parameter(self, param_name: str) -> str:
        ...
        if any(word in param_lower for word in ['price', 'cost', 'value']):
            return 'price'
        elif any(word in param_lower for word in ['volume', 'vol', 'shares', 'size']):
            return 'volume'
        elif any(word in param_lower for word in ['atr', 'ema', 'sma', 'rsi', 'macd', 'technical']):
            return 'technical'
        elif any(word in param_lower for word in ['momentum', 'trend', 'slope']):
            return 'momentum'
        elif any(word in param_lower for word in ['risk', 'stop', 'loss']):
            return 'risk'
        elif any(word in param_lower for word in ['time', 'period', 'window', 'lookback']):
            return 'timing'
        else:
            return 'technical'  # Default category

    def _determine_importance(self, param_name: str, category: str) -> str:
        """Determine parameter importance based on name and category"""
        param_lower = param_name.lower()

        # High importance parameters
        high_importance_words = ['min', 'max', 'threshold', 'atr', 'volume', 'price']
        if any(word in param_lower for word in high_importance_words):
            return 'high'

        # Medium importance parameters
        medium_importance_words = ['multiplier', 'factor', 'period', 'window']
        if any(word in param_lower for word in medium_importance_words):
            return 'medium'

        # Default importance based on category
        if category in ['price', 'volume', 'risk']:
            return 'high'
        elif category in ['technical', 'momentum']:
            return 'medium'
        else:
            return 'low'
