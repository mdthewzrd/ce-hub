...
class SmartInfrastructureFormatter:
    ...
    def __init__(self):
        super().__init__()
        self.user_requirements = {}  # Store user requirements for universe expansion
    ...
    def format_uploaded_code(self, original_code: str, user_requirements: Dict[str, Any] = None) -> str:
        ...
        # Store user requirements for universe expansion
        self.user_requirements = user_requirements if user_requirements is not None else {}
    ...
    def _add_smart_infrastructure(self, base_code: str, original_code: str) -> str:
        ...
        # Apply universe expansion if requested
        if self.user_requirements and self.user_requirements.get('full_universe', False):
            print("🌍 SMART UNIVERSE EXPANSION: Expanding ticker universe...")
            try:
                from intelligent_enhancement import expand_to_full_universe
                enhanced_code = expand_to_full_universe(enhanced_code)
                print("✅ Smart universe expansion completed")
            except Exception as e:
                print(f"⚠️ Smart universe expansion failed: {e}")
