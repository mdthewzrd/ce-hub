...
def check_high_lvl_filter_lc(df):
    """
    🎯 INDIVIDUAL SCANNER: lc_frontside_d2_extended

    Modified from original multi-scanner to calculate ONLY the lc_frontside_d2_extended pattern.
    This ensures backend detects this as an individual scanner with 100% confidence.
    """

    # 🚀 CRITICAL CHANGE: Calculate ONLY lc_frontside_d2_extended pattern
    # Original multi-scanner calculated multiple patterns - this calculates just ONE
    ...
    # 🎯 CRITICAL CHANGE: Filter for ONLY lc_frontside_d2_extended pattern
    # Original checked multiple columns - this checks only ONE
    columns_to_check = ['lc_frontside_d2_extended']

    df2 = df[df[columns_to_check].any(axis=1)]
    return df2

def filter_lc_rows(df):
    # Only filter for the lc_frontside_d2_extended pattern
...
async def main():
    ...
    Complete main function preserved from original multi-scanner.
    This ensures proper data loading, processing, and execution workflow.
    """
    global df_lc, df_sc
    ### Get Main List
    all_results = []
