# Enhanced Backside Para B Scanner
# Extended date range for proper indicator calculations
...
def fetch_daily(tkr: str, start: str, end: str) -> pd.DataFrame:
    """Fetch daily price data with extended range for indicators"""
...
def scan_symbol(symbol: str, start_date: str, end_date: str) -> List[Dict[str, Any]]:
    """Main scanning function for backside B patterns with extended data fetching"""
    try:
        # Calculate extended start date for indicator calculations
        # Need extra ~1100 days for 1000-day lookback + indicators
        start_dt = datetime.strptime(start_date, "%Y-%m-%d")
        extended_start = (start_dt - timedelta(days=1100)).strftime("%Y-%m-%d")

        # Fetch extended data for proper indicator calculations
        df = fetch_daily(symbol, extended_start, end_date)
        if df.empty:
            return []
        ...
        # Filter to target date range for pattern detection (day zero only)
        ...
        for i in range(2, len(m)):
            ...
            # Only consider patterns within the target date range (day zero dates)
            if d0 < target_start or d0 > target_end:
                continue
