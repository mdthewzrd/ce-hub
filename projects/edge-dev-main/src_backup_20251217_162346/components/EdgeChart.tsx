...
const ChartLegend: React.FC<ChartLegendProps> = ({ symbol, data, hoveredIndex, timeframe, onHover, dayNavigation }) => {
  // Comprehensive debugging to understand D+1 issue
  console.log('🏷️ ChartLegend render:', {
    symbol,
    dataLength: data.x.length,
    hoveredIndex,
    timeframe,
    dayNavigation: dayNavigation ? {
      dayOffset: dayNavigation.dayOffset,
      currentDay: dayNavigation.currentDay.toISOString(),
      currentDayFormatted: dayNavigation.currentDay.toLocaleDateString()
    } : 'undefined'
  });

  // Show hovered bar if available, otherwise show most recent bar
  // For Day 0, ensure we don't show future data beyond the target date
  const defaultIndex = data.x.length - 1;
  let displayIndex = hoveredIndex ?? defaultIndex;

  // 🔍 DEBUG: Log detailed date information
  if (data.x.length > 0) {
    const lastDataDate = data.x[data.x.length - 1];
    const lastDataDateObj = new Date(lastDataDate);
    console.log('🔍 DATE ANALYSIS:', {
      lastDataDateRaw: lastDataDate,
      lastDataDateObj: lastDataDateObj.toISOString(),
      lastDataDateFormatted: lastDataDateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'America/New_York' }),
      defaultIndex: defaultIndex,
      displayIndex: displayIndex,
      dayNavigationOffset: dayNavigation?.dayOffset,
      dayNavigationCurrentDay: dayNavigation?.currentDay?.toISOString()
    });
  }

  // 🔧 CRITICAL FIX: Legend should ALWAYS use the same data as the chart
  // The API already filters the data properly, so just use the last available bar
  // No additional filtering needed - chart and legend are now synchronized
  ...
  // Format date and time based on timeframe - FIXED to prevent timezone conversion issues
  const formatDateTime = (dateStr: string) => {
    try {
      // Parse the ISO date string directly to avoid timezone conversion issues
      // dateStr format: "2025-06-11T00:00:00"
      ...
      if (dateMatch) {
        ...
        if (timeframe === 'day') {
          // For daily timeframe, show only the date
          return `${monthName} ${dayNum}, ${year}`;
        } else {
          // For intraday timeframes, show both date and time
          return {
      ...
      // Fallback: Parse as UTC date to avoid timezone conversion issues
      // The API returns timestamps in ET market time, but we want to display them consistently
      const date = new Date(dateStr);

      // Get date components in ET to match market data
...
interface EdgeChartProps {
  ...
  className?: string;
  baseDate?: string; // Date from scan result to center chart around
    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-sm text-gray-400">
          ...
          {template.extendedHours && (
            <div className="mt-1 text-xs text-yellow-400">
              🌐 GLOBAL Template • 🕐 Extended hours (4am-8pm) • 🛡️ Holiday filtered
            </div>
          )}
        </div>
      </div>
      ...
      <div className="relative w-full h-full">
        <Plot
          ...
          // CRITICAL: Minimal initialization to prevent conflicts
