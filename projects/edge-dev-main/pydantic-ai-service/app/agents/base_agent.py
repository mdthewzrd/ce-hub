...
def get_model(model_name: str = None):
    """
    Get the appropriate PydanticAI model based on provider settings
    ...
    if provider == "openrouter":
        # Configure OpenRouter using OpenAI-compatible interface
        if not settings.OPENROUTER_API_KEY:
            logger.warning("⚠️ OPENROUTER_API_KEY not set, falling back to direct model specification")
            return OpenAIModel(model=model, api_key="")
        ...
        logger.info(f"🤖 Using default OpenAI-compatible interface with model: {model}")
...
class BaseAgent(ABC):
    ...
    def __init__(self, agent_type: AgentType, model: str = None):
        ...
        self.model = get_model(self.model_name)  # Get configured PydanticAI model
        ...
        model_provider = settings.LLM_PROVIDER.upper()
        self.logger.info(f"Initializing {agent_type.value} agent with {model_provider} model: {self.model_name}")
