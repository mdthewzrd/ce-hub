#!/usr/bin/env python3
"""
LC Frontside D2 Extended Scanner - Manually Split
2-day pattern recognition for late-close continuation with EMA distance focus
"""
import pandas as pd
import requests
import numpy as np
...
def calculate_indicators(df):
    """Calculate all technical indicators needed for LC D2 Extended pattern"""
...
def lc_frontside_d2_extended_scan(df):
    """
    LC Frontside D2 Extended Pattern Logic
    2-day ascending pattern with strong EMA distance focus
    """

    # Calculate all indicators
    df = calculate_indicators(df)

    # LC Frontside D2 Extended Pattern
    pattern = (
        # 2-day ascending highs and lows
        (df['h'] >= df['h1']) &
        (df['l'] >= df['l1']) &

        # Price tier-based high percentage change requirements with 5-day high vs 20-day low distance
        (
            ((df['high_pct_chg'] >= 0.5) & (df['c_ua'] >= 5) & (df['c_ua'] < 15) & (df['highest_high_5_dist_to_lowest_low_20_pct_1'] >= 2.5)) |
            ((df['high_pct_chg'] >= 0.3) & (df['c_ua'] >= 15) & (df['c_ua'] < 25) & (df['highest_high_5_dist_to_lowest_low_20_pct_1'] >= 2)) |
            ((df['high_pct_chg'] >= 0.2) & (df['c_ua'] >= 25) & (df['c_ua'] < 50) & (df['highest_high_5_dist_to_lowest_low_20_pct_1'] >= 1.5)) |
            ((df['high_pct_chg'] >= 0.15) & (df['c_ua'] >= 50) & (df['c_ua'] < 90) & (df['highest_high_5_dist_to_lowest_low_20_pct_1'] >= 1)) |
            ((df['high_pct_chg'] >= 0.1) & (df['c_ua'] >= 90) & (df['highest_high_5_dist_to_lowest_low_20_pct_1'] >= 0.75))
        ) &
        ...
        # Strong EMA distance requirements (key differentiator for D2 Extended)
    ...
    if len(results) > 0:
        results['scanner_type'] = 'LC_Frontside_D2_Extended'
        results['signal_strength'] = 'High'
        results['pattern_description'] = '2-day ascending with strong EMA distance'
...
async def run_scanner():
    ...
    print("🚀 Starting LC Frontside D2 Extended Scanner...")
    ...
    # Run the scanner
    results = lc_frontside_d2_extended_scan(filtered_df)
    ...
    if len(results) > 0:
        ...
        # Save results
        results.to_csv(f"lc_d2_extended_results_{DATE.replace('-', '')}.csv", index=False)
        print(f"\n💾 Results saved to: lc_d2_extended_results_{DATE.replace('-', '')}.csv")

    else:
        print("\n📊 No matches found for LC Frontside D2 Extended pattern")
