...
# Authentication
security = HTTPBearer(auto_error=False)  # Allow optional authentication for mock mode
...
async def get_current_user(
    ...
    try:
        ...
        if settings.mock_auth or settings.debug:
            return {
        ...
        # Require credentials for production authentication
        if not credentials:
            raise HTTPException(
...
# Lightweight AI context for operations that don't need database
class LightweightAIContext:
    """AI context without database dependency for mock/development operations"""

    def __init__(self, archon: ArchonClient):
        self.archon = archon
        # Mock user data for development
        self.user_id = "mock_user_123"
        self.workspace_id = "mock_workspace_456"


async def get_lightweight_ai_context(
    archon: ArchonClient = Depends(get_archon)
) -> LightweightAIContext:
    """
    Dependency for AI operations that don't need database access

    This is useful for development/testing when database isn't available
    but Archon integration is still needed.
    """
    return LightweightAIContext(archon=archon)
