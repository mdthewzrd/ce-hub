...
def check_high_lvl_filter_lc(df):
    """
    🔧 SINGLE SCANNER: LC Frontside D2 Extended Only
    """
    # Only calculate the lc_frontside_d2_extended pattern
...
async def main(start_date: str, end_date: str):
    ...
    Single pattern scanner focused only on lc_frontside_d2_extended
    """
    print(f"🎯 LC Frontside D2 Extended Scanner executing with date range: {start_date} to {end_date}")
    ...
    # Apply the LC Frontside D2 Extended filter
    ...
    print(f"🎯 LC Frontside D2 Extended Scanner Results:")
    print(df_lc[['date', 'ticker']])

    # Return results as list of dictionaries for the API
    return df_lc[['date', 'ticker', 'lc_frontside_d2_extended']].to_dict('records')