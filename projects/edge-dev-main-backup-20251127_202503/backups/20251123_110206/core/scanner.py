...
def compute_indicators1(df):
    """
    🔧 CRITICAL FIX: Ensure all numeric columns are properly converted to prevent
    '>' not supported between instances of 'str' and 'int' errors
    """

    # CRITICAL: Convert all numeric columns to proper types before any operations
    numeric_columns = [
        'o', 'h', 'l', 'c', 'v', 'vw', 'n',  # Basic OHLCV data
        'gap_pct', 'gap_atr', 'gap_atr1', 'gap_atr2',  # Gap metrics
        'high_pct_chg', 'high_pct_chg1', 'high_pct_chg2',  # Price change metrics
        'high_chg_atr', 'high_chg_atr1', 'high_chg_atr2',  # ATR normalized metrics
        'close_range', 'close_range1', 'close_range2',  # Range metrics
        'c_ua', 'c_ua1', 'c_ua2', 'c_ua3',  # Adjusted close
        'v_ua', 'v_ua1', 'v_ua2', 'v_ua3',  # Adjusted volume
        'dol_v', 'dol_v1', 'dol_v2', 'dol_v3',  # Dollar volume
        'rvol', 'rvol1', 'rvol2',  # Relative volume
        'ema9', 'ema20', 'ema50', 'ema200',  # EMAs
        'ema9_1', 'ema20_1', 'ema50_1', 'ema200_1',  # Shifted EMAs
        'dist_h_9ema_atr1', 'dist_9ema_atr1',  # EMA distance metrics
        'atr', 'true_range',  # Volatility metrics
        'range', 'range1', 'range2',  # Range metrics
        'highest_high_250'  # Lookback metrics
    ]

    # Convert all numeric columns to float, handling errors gracefully
    for col in numeric_columns:
        if col in df.columns:
            try:
                ...
                # If conversion fails completely, ensure it's at least float type
                try:
                    df[col] = df[col].astype(float, errors='ignore')
                except:
                    df[col] = 0.0  # Ultimate fallback

    print(f"✅ NUMERIC CONVERSION: Converted {len([c for c in numeric_columns if c in df.columns])}/{len(numeric_columns)} numeric columns")
