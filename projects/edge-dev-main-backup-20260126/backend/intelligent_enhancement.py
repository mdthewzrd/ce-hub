...
def enhance_scanner_infrastructure(code: str, pure_execution_mode: bool = False) -> str:
    ...
    - Only enhances infrastructure components
    """
    # 🔧 CRITICAL FIX: Remove thinking tags that some AI models add to generated code
    # These tags cause Python syntax errors when the code is executed
    # This must happen BEFORE any processing to ensure all code is cleaned
    import re
    # AGGRESSIVE removal of all thinking tag variations
    # Remove both opening and closing tags, with or without attributes, with or without newlines
    code = re.sub(r'<think>.*?</think>', '', code, flags=re.DOTALL | re.IGNORECASE)  # Remove entire think blocks
    code = re.sub(r'</think>\n?', '', code, flags=re.IGNORECASE)  # Remove closing tags
    code = re.sub(r'<think[^>]*>\n?', '', code, flags=re.IGNORECASE)  # Remove opening tags with attributes
    code = re.sub(r'<think>\n?', '', code, flags=re.IGNORECASE)  # Remove simple opening tags
    code = code.strip()

    print(f"🧹 Thinking tag cleanup applied")
