...
class ExecuteProjectRequest(BaseModel):
    ...
    scanner_code: Optional[str] = Field(None, description="Uploaded scanner code for direct execution")
...
async def execute_project(project_id: str, request: ExecuteProjectRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Check if this is an uploaded scanner project (no structured scanners)
        # In that case, we'll handle it differently by looking for uploaded code
        if not project.scanners:
            # This might be an uploaded scanner project - redirect to main execution logic
            # Check if we have scanner_code in the request to handle uploaded scanners
            if request.scanner_code:
                # Forward to the main execution logic for uploaded scanners
                from ..main import execute_scan_robust

                # Extract date range
                start_date = request.date_range.get('start_date', '2025-01-01')
                end_date = request.date_range.get('end_date', '2025-11-01')

                # Execute the uploaded scanner directly
                from main import execute_uploaded_scanner_robust
                scan_results = await execute_uploaded_scanner_robust(
                    uploaded_code=request.scanner_code,
                ...
                return ExecutionResponse(
                    ...
                    total_signals=len(scan_results) if scan_results else 0,
                    scanner_results={"uploaded_scanner": len(scan_results) if scan_results else 0}
                )

            # If no scanner_code and no structured scanners, then it's an error
