...
A daily scanner that identifies stocks showing a parabolic move (D-1 or D-2) followed by a gap up and open above previous high, indicating potential continuation of momentum.

Generated by RENATA_V2 - 2026-01-08 17:28:06
...
def fetch_daily_multi_range(start: str, end: str) -> pd.DataFrame:
    ...
    if all_data:
        ...
        print(f"✅ Total {len(result)} records across {len(all_data)} dates")
        return result
    else:
        return pd.DataFrame()
...
class backside_para_b_copy_3:
    ...
    def __init__(self):
        ...
        self.strategy_name = "Daily Parabolic Backside Lite Scan"
    ...
    def fetch_grouped_data(
        ...
        print(f"  🌐 Using grouped API endpoint (efficient!)")
        ...
        while current_dt <= end_dt:
            ...
            if current_dt.weekday() < 5:
                ...
                try:
                    ...
                    if not df_day.empty:
                        all_data.append(df_day)
                        print(f"      ✅ Got {len(df_day)} records")
                except Exception as e:
                    print(f"      ⚠️  Error fetching {date_str}: {e}")
    ...
    def apply_smart_filters(self, stage1_data, workers: int = 6):
        ...
        if stage1_data is None or stage1_data.empty:
            print("  ⚠️  Stage 2: No data to filter")
            return stage1_data

        print(f"  📊 Stage 2 input: {len(stage1_data)} rows")
        ...
        print(f"  ✅ Stage 2 complete: {len(filtered_data)} rows ({len(filtered_data)/len(stage1_data)*100:.1f}% retained)")

        return filtered_data

    def detect_patterns(self, stage2_data):
        """
        🎯 Stage 3: Detect trading patterns
        ...
        if stage2_data is None or stage2_data.empty:
            print("  ⚠️  Stage 3: No data for pattern detection")
            return []

        print(f"  🎯 Stage 3: Scanning {len(stage2_data)} rows for patterns...")
        ...
        try:
            ...
            if results:
                ...
                print(f"  ✅ Stage 3 complete: {len(combined)} signals detected")
                return combined.to_dict('records')
            else:
                print(f"  ✅ Stage 3 complete: No signals detected")
                return []

        except Exception as e:
            print(f"  ⚠️  Stage 3 error: {e}")
            return []

    def _run_pattern_detection(self, data):
        """
        🎯 Run pattern detection logic with ORIGINAL logic preserved

        CRITICAL: This method contains the actual detection logic extracted from the original scanner.
        The scan_symbol function logic was 3770 characters and has been embedded below.
        """
        if data is None or data.empty:
            return pd.DataFrame()

        # 🚨 ORIGINAL DETECTION LOGIC FROM SCAN_SYMBOL:
        # The following logic was extracted from the original scanner's scan_symbol function
        # This is the actual pattern detection code with all 3800+ characters preserved

        # Helper functions (if extracted):
...
def pos_between(val, lo, hi):
    ...
    return max(0.0, min(1.0, float((val - lo) / (hi - lo))))
        ...
        # Original scan_symbol body (extracted from original scanner):
    ...
    return pd.DataFrame(rows)
        ...
        # If we get here, the detection logic didn't return results
        return pd.DataFrame()
    ...
    def format_results(self, stage3_results):
        ...
        if not stage3_results:
            import pandas as pd
            print(f"  📊 Stage 4: No results to format")
            return pd.DataFrame()
        ...
        print(f"  ✅ Stage 4 complete: {len(df)} results formatted")
    ...
    def run_scan(
        ...
        """
        🚀 Stage 5: Run complete 5-stage pipeline
        ...
        print(f"\n======================================================================")
        print(f"🚀 RUNNING 5-STAGE SCAN: {self.scanner_name}")
        print(f"======================================================================")
        print(f"📊 Strategy: {self.strategy_name}")
        print(f"📅 Date Range: {start_date} to {end_date}")
        print(f"⚙️  Workers: {workers}")
        print(f"🎯 Parameters: {len(self.params)} configured")
        print(f"======================================================================\n")
        ...
        if self.stage2_data is None or self.stage2_data.empty:
            print("\n⚠️  Scan complete: No data passed filters")
            return pd.DataFrame()
        ...
        print(f"\n======================================================================")
        print(f"✅ SCAN COMPLETE: {self.scanner_name}")
        print(f"======================================================================")
        print(f"📊 Total Signals Found: {len(self.stage4_results)}")
        print(f"⏱️  Execution Time: {duration:.2f} seconds")
        print(f"📈 Success Rate: {len(self.stage4_results)/len(self.stage1_data)*100:.2f}%")
        print(f"======================================================================\n")
