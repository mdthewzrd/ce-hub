...
A daily-only scanner that identifies A+ parabolic backside setups with specific volume and momentum criteria. Looks for stocks that have formed a strong parabolic move and are setting up for a potential backside continuation play.

Generated by RENATA_V2 - 2026-01-21 13:55:00
    ...
    def _process_ticker_optimized_pre_sliced(self, ticker_data: tuple):
        ...
        for i in range(2, len(ticker_df)):
            ...
            try:
                    ...
                    # Calculate score (normalized 0-100 based on multiple factors)
                    score_components = []
                    if pd.notna(r0["gap_over_atr"]):
                        score_components.append(min(r0["gap_over_atr"] * 10, 40))  # Max 40 pts for gap
                    if pd.notna(volsig_max):
                        score_components.append(min(volsig_max * 15, 30))  # Max 30 pts for volume
                    if pd.notna(pos_abs_prev):
                        score_components.append(min((1 - pos_abs_prev) * 30, 30))  # Max 30 pts for position
                    score = round(sum(score_components), 1) if score_components else 50.0
                    ...
                    all_rows.append({
                        ...
                        "D1Vol(shares)": int(r1["volume"]) if pd.notna(r1["volume"]) else np.nan,   # D-1 absolute volume
                        "D0Vol(shares)": int(r0["volume"]) if pd.notna(r0["volume"]) else np.nan,   # D0 absolute volume
                        "D1Vol/Avg": round(float(d1_vol_mult), 2) if pd.notna(d1_vol_mult) else np.nan,
                        "VolSig(max D-1,D-2)/Avg": round(float(volsig_max), 2) if pd.notna(volsig_max) else np.nan,
                        "Gap/atr": round(float(r0["gap_over_atr"]), 2),
                        "Gap%": round(float(r0["gap_over_atr"]) * 100, 1) if pd.notna(r0["gap_over_atr"]) else np.nan,  # Gap as percentage
...
def run_scan(d0_start=None, d0_end=None):
    ...
    # Return FULL results with all columns (not just ticker+date)
    return results
