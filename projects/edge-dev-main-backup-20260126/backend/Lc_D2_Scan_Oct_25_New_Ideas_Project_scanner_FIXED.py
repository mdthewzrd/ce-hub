"""
lc_d2_scan_oct_25_new_ideas_6

Multi-Pattern Scanner - 6 patterns

Generated by RENATA_V2 - Multi-Scanner to V31 Transformation
Date Range: 2024-01-01 to 2024-12-31
Patterns: lc_frontside_d2_extended, lc_frontside_d2_extended_1, big_d2, lc_frontside_d3_extended_1, lc_frontside_d4_para, lc_frontside_d3_extended_2, lc_frontside_d3_extended_3
...
class lc_d2_scan_oct_25_new_ideas_6:
    """
    Multi-Pattern Scanner with 6 pattern detectors
        ...
        self.pattern_assignments = [
    {
        ...
        "logic": "(df['h']>= df['h1']) & (df['h1']>= df['h2']) & (df['l']>= df['l1']) & (df['l1']>= df['l2']) & ((df['high_pct_chg1']>= 0.3) & (df['high_pct_chg']>= 0.3) & (df['c_ua']>= 1) & (df['c_ua']< 15) & (df['h_dist_to_lowest_low_20_pct']>= 2.5) | (df['high_pct_chg1']>= 0.2) & (df['high_pct_chg']>= 0.2) & (df['c_ua']>= 15) & (df['c_ua']< 25) & (df['h_dist_to_lowest_low_20_pct']>= 2) | (df['high_pct_chg1']>= 0.1) & (df['high_pct_chg']>= 0.1) & (df['c_ua']>= 25) & (df['c_ua']< 50) & (df['h_dist_to_lowest_low_20_pct']>= 1.5) | (df['high_pct_chg1']>= 0.07) & (df['high_pct_chg']>= 0.07) & (df['c_ua']>= 50) & (df['c_ua']< 90) & (df['h_dist_to_lowest_low_20_pct']>= 1) | (df['high_pct_chg1']>= 0.05) & (df['high_pct_chg']>= 0.05) & (df['c_ua']>= 90) & (df['h_dist_to_lowest_low_20_pct']>= 0.75)) & (df['high_chg_atr1']>= 0.7) & (df['c1']>= df['o1']) & (df['dist_h_9ema_atr1']>= 1.5) & (df['dist_h_20ema_atr1']>= 2) & (df['v_ua1']>= 10000000) & (df['dol_v1']>= 100000000) & (df['high_chg_atr']>= 1) & (df['c']>= df['o']) & (df['dist_h_9ema_atr']>= 1.5) & (df['dist_h_20ema_atr']>= 2) & (df['v_ua']>= 10000000) & (df['dol_v']>= 100000000) & ((df['high_chg_atr']>= 1) | (df['high_chg_atr1']>= 1)) & (df['h_dist_to_highest_high_20_2_atr']>= 2.5) & ((df['h']>= df['highest_high_20']) & (df['ema9']>= df['ema20']) & (df['ema20']>= df['ema50']))"
    },
    {
        "name": "lc_frontside_d2_extended",
        "logic": "(df['h']>= df['h1']) & (df['l']>= df['l1']) & ((df['high_pct_chg']>= 0.5) & (df['c_ua']>= 1) & (df['c_ua']< 15) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 2.5) | (df['high_pct_chg']>= 0.3) & (df['c_ua']>= 15) & (df['c_ua']< 25) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 2) | (df['high_pct_chg']>= 0.2) & (df['c_ua']>= 25) & (df['c_ua']< 50) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 1.5) | (df['high_pct_chg']>= 0.15) & (df['c_ua']>= 50) & (df['c_ua']< 90) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 1) | (df['high_pct_chg']>= 0.1) & (df['c_ua']>= 90) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 0.75)) & (df['high_chg_atr']>= 2) & (df['c']>= df['o']) & (df['dist_h_9ema_atr']>= 2) & (df['dist_h_20ema_atr']>= 3) & (df['v_ua']>= 10000000) & (df['dol_v']>= 100000000) & (df['dist_l_9ema_atr']>= 2) & (df['h_dist_to_highest_high_20_1_atr']>= 1) & (df['dol_v_cum5_1']>= 200000000) & ((df['h']>= df['highest_high_20']) & (df['ema9']>= df['ema20']) & (df['ema20']>= df['ema50']))"
    },
    {
        "name": "lc_frontside_d2_extended_1",
        "logic": "(df['h']>= df['h1']) & (df['l']>= df['l1']) & ((df['high_pct_chg']>= 1) & (df['c_ua']>= 1) & (df['c_ua']< 15) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 2.5) | (df['high_pct_chg']>= 0.5) & (df['c_ua']>= 15) & (df['c_ua']< 25) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 2) | (df['high_pct_chg']>= 0.3) & (df['c_ua']>= 25) & (df['c_ua']< 50) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 1.5) | (df['high_pct_chg']>= 0.2) & (df['c_ua']>= 50) & (df['c_ua']< 90) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 1) | (df['high_pct_chg']>= 0.15) & (df['c_ua']>= 90) & (df['highest_high_5_dist_to_lowest_low_20_pct_1']>= 0.75)) & (df['high_chg_atr']>= 1.5) & (df['c']>= df['o']) & (df['dist_h_9ema_atr']>= 2) & (df['dist_h_20ema_atr']>= 3) & (df['v_ua']>= 10000000) & (df['dol_v']>= 100000000) & (df['h_dist_to_highest_high_20_1_atr']>= 1) & (df['dol_v_cum5_1']>= 200000000) & ((df['h']>= df['highest_high_20']) & (df['ema9']>= df['ema20']) & (df['ema20']>= df['ema50']))"
    },
    {
        "name": "lc_frontside_d3_extended_2",
        "logic": "(df['h']>= df['h1']) & (df['h1']>= df['h2']) & ((df['high_pct_chg']>= 0.15) & (df['c_ua']>= 1) & (df['c_ua']< 15) & (df['h_dist_to_lowest_low_20_pct']>= 1.5) | (df['high_pct_chg']>= 0.1) & (df['c_ua']>= 15) & (df['c_ua']< 25) & (df['h_dist_to_lowest_low_20_pct']>= 1) | (df['high_pct_chg']>= 0.07) & (df['c_ua']>= 25) & (df['c_ua']< 50) & (df['h_dist_to_lowest_low_20_pct']>= 0.7) | (df['high_pct_chg']>= 0.05) & (df['c_ua']>= 50) & (df['c_ua']< 90) & (df['h_dist_to_lowest_low_20_pct']>= 0.5) | (df['high_pct_chg']>= 0.03) & (df['c_ua']>= 90) & (df['h_dist_to_lowest_low_20_pct']>= 0.2)) & (df['high_chg_atr1']>= 0.5) & (high_chg_from_pdc_atr1 >= 1) & (df['gap_atr1']>= 0.2) & (df['close_range1']>= 0.6) & (df['c1']>= df['o1']) & (df['dist_h_9ema_atr1']>= 2) & (df['dist_h_20ema_atr1']>= 3) & (df['v_ua1']>= 10000000) & (df['dol_v1']>= 100000000) & (df['high_chg_atr']>= 1) & (high_chg_from_pdc_atr >= 1) & (df['gap_atr']>= 0.2) & (df['close_range']>= 0.6) & (df['c']>= df['o']) & (df['dist_h_9ema_atr']>= 3) & (dist_h_50ema_atr >= 5) & (df['v_ua']>= 10000000) & (df['dol_v']>= 100000000) & (h_dist_to_lowest_low_20_atr >= 5) & (h_dist_to_lowest_low_5_atr >= 3) & ((df['h']>= df['highest_high_20']) & (df['ema9']>= df['ema20']) & (df['ema20']>= df['ema50']))"
    },
    {
        "name": "lc_frontside_d3_extended_3",
        "logic": "(df['h']>= df['h1']) & (df['h1']>= df['h2']) & ((df['high_pct_chg']>= 0.15) & (df['c_ua']>= 1) & (df['c_ua']< 15) & (df['h_dist_to_lowest_low_20_pct']>= 1.5) | (df['high_pct_chg']>= 0.1) & (df['c_ua']>= 15) & (df['c_ua']< 25) & (df['h_dist_to_lowest_low_20_pct']>= 1) | (df['high_pct_chg']>= 0.07) & (df['c_ua']>= 25) & (df['c_ua']< 50) & (df['h_dist_to_lowest_low_20_pct']>= 0.7) | (df['high_pct_chg']>= 0.05) & (df['c_ua']>= 50) & (df['c_ua']< 90) & (df['h_dist_to_lowest_low_20_pct']>= 0.5) | (df['high_pct_chg']>= 0.03) & (df['c_ua']>= 90) & (df['h_dist_to_lowest_low_20_pct']>= 0.2)) & (df['high_chg_atr1']>= 0.3) & (high_chg_from_pdc_atr1 >= 1) & (df['close_range1']>= 0.5) & (df['c1']>= df['o1']) & (df['dist_h_9ema_atr1']>= 1) & (df['high_chg1']>= high_chg * 0.4) & (df['v_ua1']>= 10000000) & (df['dol_v1']>= 100000000) & (df['high_chg_atr']>= 0.6) & (high_chg_from_pdc_atr >= 1) & (df['gap_atr']>= 0.2) & (df['close_range']>= 0.4) & (df['c']>= df['o']) & (df['dist_h_9ema_atr']>= 1.5) & (df['dist_h_20ema_atr']>= 3) & (dist_h_50ema_atr >= 5) & (df['v_ua']>= 10000000) & (df['dol_v']>= 100000000) & (h_dist_to_lowest_low_20_atr >= 5) & (h_dist_to_lowest_low_5_atr >= 3) & ((df['h']>= df['highest_high_20']) & (df['ema9']>= df['ema20']) & (df['ema20']>= df['ema50']))"
    },
    {
        "name": "lc_frontside_d4_para",
        "logic": "(df['h']>= df['h1']) & (df['h1']>= df['h2']) & ((df['high_pct_chg']>= 0.15) & (df['c_ua']>= 1) & (df['c_ua']< 15) & (df['h_dist_to_lowest_low_20_pct']>= 1.5) | (df['high_pct_chg']>= 0.1) & (df['c_ua']>= 15) & (df['c_ua']< 25) & (df['h_dist_to_lowest_low_20_pct']>= 1) | (df['high_pct_chg']>= 0.07) & (df['c_ua']>= 25) & (df['c_ua']< 50) & (df['h_dist_to_lowest_low_20_pct']>= 0.7) | (df['high_pct_chg']>= 0.05) & (df['c_ua']>= 50) & (df['c_ua']< 90) & (df['h_dist_to_lowest_low_20_pct']>= 0.5) | (df['high_pct_chg']>= 0.03) & (df['c_ua']>= 90) & (df['h_dist_to_lowest_low_20_pct']>= 0.2)) & (dist_h_9ema_atr2 >= 1) & (dist_h_9ema_atr3 >= 1) & (dist_h_9ema_atr4 >= 1) & (df['c2']>= df['o2']) & (dist_h_20ema_atr2 >= 1.5) & (df['high_chg_atr1']>= 0.3) & (high_chg_from_pdc_atr1 >= 1) & (df['gap_atr1']>= 0) & (df['close_range1']>= 0.3) & (df['c1']>= df['o1']) & (df['dist_h_9ema_atr1']>= 1.5) & (df['dist_h_20ema_atr1']>= 3) & (df['v_ua1']>= 10000000) & (df['dol_v1']>= 100000000) & (df['high_chg_atr']>= 0.3) & (high_chg_from_pdc_atr >= 1) & (df['gap_atr']>= 0.25) & (df['close_range']>= 0.3) & (df['c']>= df['o']) & (df['dist_h_9ema_atr']>= 2) & (dist_h_50ema_atr >= 5) & (df['v_ua']>= 10000000) & (df['dol_v']>= 100000000) & (h_dist_to_lowest_low_20_atr >= 5) & (h_dist_to_lowest_low_5_atr >= 3) & ((df['h']>= df['highest_high_20']) & (df['ema9']>= df['ema20']) & (df['ema20']>= df['ema50']))"
    }
]
        ...
        print(f"🚀 MULTI-PATTERN SCANNER: lc_d2_scan_oct_25_new_ideas_6")
    ...
    def detect_patterns(self, stage2_data: pd.DataFrame) -> pd.DataFrame:
        ...
        This method detects all 6 patterns:
    - lc_frontside_d3_extended_1
    - lc_frontside_d2_extended
    - lc_frontside_d2_extended_1
    - lc_frontside_d3_extended_2
...
if __name__ == "__main__":
    ...
    scanner = lc_d2_scan_oct_25_new_ideas_6()
