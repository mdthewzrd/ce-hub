...
async def execute_uploaded_scanner_direct(
    ...
    try:
        ...
        if 'scan_symbol' in all_function_matches:
        ...
        if is_backside_scanner and has_user_function and user_function_name:
...
async def execute_generic_pattern_simple(code: str, start_date: str, end_date: str, progress_callback):
    ...
    def run_generic_sync():
        ...
        try:
            ...
            # 🔧 STEP 2: Apply PROVEN date range logic for Backside B scanners
            print(f"🎯 Applying PROVEN date range logic: fetch from 2021, display {start_date} to {end_date}")
            ...
            if output_text.strip():
                ...
                # Look for the start of the actual data table
                data_start = -1
                header_line = ""
                ...
                if data_start >= 0 and data_start < len(lines):
                    ...
                    for line in lines[data_start:]:
                        ...
                        if line and not line.startswith('─') and not line.startswith('='):
                            # Parse table row with proper spacing
                            parts = line.split()
                            if len(parts) >= 12:  # Backside B has many columns
                                try:
                                    # Extract ticker and date from the data row
                                    ticker = parts[0]
                                    date_str = parts[1]

                                    # Validate ticker (should be uppercase letters) and date (should be YYYY-MM-DD)
                                    if (ticker.isupper() and len(ticker) <= 5 and
                                        len(date_str) == 10 and date_str.count('-') == 2):

                                        # Extract additional meaningful data
                                        trigger = parts[2] if len(parts) > 2 else "D-1"
                                        pos_abs = float(parts[3]) if len(parts) > 3 and parts[3].replace('.', '').replace('-', '').isdigit() else 0.0
                                        body_atr = float(parts[4]) if len(parts) > 4 and parts[4].replace('.', '').replace('-', '').isdigit() else 0.0
                                        volume = int(parts[5].replace(',', '')) if len(parts) > 5 and parts[5].replace(',', '').isdigit() else 0

                                        results.append({
                                            'ticker': ticker,
                                            'date': date_str,
                                            'signal': f"Backside B+ ({trigger})",
                                            'price': 0.0,  # Price not in table output
                                            'volume': volume,
                                            'score': body_atr,  # Use Body/ATR as score
                                            'trigger': trigger,
                                            'pos_abs': pos_abs,
                                            'scanner_type': 'A+ Daily Parabolic'
                                        })
                                        print(f"✅ Parsed data row: {ticker} {date_str} vol={volume}")
                                except Exception as e:
                                    print(f"⚠️  Error parsing line: {line} - {e}")
                                    continue
                else:
                    # Fallback to simple parsing if no header found
                    print(f"⚠️  No header found, using fallback parsing")
                    for line in lines:
                        line = line.strip()
                        if line and ' ' in line and not line.startswith('Ticker'):
                            parts = line.split()
                            if len(parts) >= 2:
                                symbol = parts[0]
                                date_str = parts[1]
                                # Basic validation
                                if (symbol.isupper() and len(symbol) <= 5 and
                                    len(date_str) == 10 and date_str.count('-') == 2):
                                    results.append({
                                        'symbol': symbol,
                                        'date': date_str,
