...
class SophisticatedCodePreservationFormatter:
    ...
    def extract_sophisticated_logic(self, code: str) -> Dict[str, Any]:
        ...
        if self.scanner_type == "a_plus":
            ...
            # For custom or unknown scanners, extract generic sophisticated logic
            self._extract_generic_sophisticated_logic(code, preserved)
    ...
    def _extract_a_plus_logic(self, code: str, preserved: Dict[str, Any]):
        ...
        """
        # Extract main scan function - try multiple patterns
        scan_function_match = re.search(
            r'def scan_daily_para\(.*?\):.*?return df_m\.loc\[cond\]',
            code, re.DOTALL
        )
        if not scan_function_match:
            # Try alternative pattern for backside-style scanners
            scan_function_match = re.search(
                r'def scan_symbol\(.*?\):.*?return pd\.DataFrame\(rows\)',
                code, re.DOTALL
            )
        if scan_function_match:
            preserved['scan_daily_para_function'] = scan_function_match.group(0)

        # Extract compute_all_metrics function - try multiple patterns
        compute_metrics_match = re.search(
            r'def compute_all_metrics\(.*?\):.*?return df',
            code, re.DOTALL
        )
        if not compute_metrics_match:
            # Try alternative pattern for backside-style scanners
            compute_metrics_match = re.search(
                r'def add_daily_metrics\(.*?\):.*?return m',
                code, re.DOTALL
            )
        ...
        # Also extract backside-style helper functions
        backside_functions = ['fetch_daily', '_mold_on_row', 'abs_top_window', 'pos_between']
        for func_name in backside_functions:
            func_match = re.search(
        ...
        # Extract symbol list - try multiple patterns
        symbols_match = re.search(
            r'symbols = \[(.*?)\]',
            code, re.DOTALL
        )
        if not symbols_match:
            # Try SYMBOLS pattern for backside-style scanners
            symbols_match = re.search(
                r'SYMBOLS = \[(.*?)\]',
                code, re.DOTALL
            )
        ...
        # Extract parameter dictionary (P) for backside-style scanners
        p_params_match = re.search(
            r'P = \{(.*?)\}',
            code, re.DOTALL
        )
        if p_params_match:
            parameter_patterns['P'] = f"P = {{{p_params_match.group(1)}}}"
...
'''
    ...
    def _extract_generic_sophisticated_logic(self, code: str, preserved: Dict[str, Any]) -> None:
        """
        Extract generic sophisticated patterns from unknown scanner types
        """
        # Extract ALL user functions (not just scan functions)
        all_functions = {}
        function_pattern = r'def\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\([^)]*\):.*?(?=def|\Z)'
        ...
        preserved['all_custom_functions'] = all_functions

        # Also store the first function as the main function
        if all_functions:
            first_func_name = list(all_functions.keys())[0]
            preserved['main_function'] = all_functions[first_func_name]

        # Extract scan function patterns (for backward compatibility)
        scan_functions = re.findall(r'def\s+scan_?\w*\([^)]*\):', code)
        preserved['scan_functions'] = scan_functions

        # Extract sophisticated variable patterns
        complex_vars = re.findall(r'([a-zA-Z_][a-zA-Z0-9_]*)\s*=.*(?:\.ewm\(|\.rolling\(|\.shift\(|\.diff\()', code)
        preserved['complex_variables'] = list(set(complex_vars))

        # Extract data access patterns
        data_access = re.findall(r'data\[\'([^\']+)\'\]', code)
        preserved['data_columns'] = list(set(data_access))

        # Extract condition patterns
        conditions = re.findall(r'if\s+(.*?):', code)
        preserved['conditions'] = conditions[:10]  # Limit to first 10 conditions

        # Extract return patterns
        return_patterns = re.findall(r'return\s+(.*?)(?=\n|$)', code)
        preserved['return_patterns'] = return_patterns
