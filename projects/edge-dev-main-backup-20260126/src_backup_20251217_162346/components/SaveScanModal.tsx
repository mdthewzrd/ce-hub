...
export const SaveScanModal: React.FC<SaveScanModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('🔄 Saving scan with data:', scanData);

      await onSave(scanData);

      // Show browser notification on successful save
      console.log('✅ Scan saved successfully, showing notification...');
      if ('Notification' in window) {
        if (Notification.permission === 'granted') {
          new Notification('Scan Saved Successfully', {
            body: `${currentResults.length} results saved for "${scanName.trim()}"`,
            icon: '/favicon.ico',
            tag: 'scan-saved'
          });
  ...
  console.log('🎭 SaveScanModal rendering - isOpen:', isOpen);

  if (!isOpen) {
    console.log('🚫 SaveScanModal: isOpen is false, not rendering');
    return null;
  }

  console.log('✅ SaveScanModal: isOpen is true, rendering modal');

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4" data-save-scan-modal="true">
