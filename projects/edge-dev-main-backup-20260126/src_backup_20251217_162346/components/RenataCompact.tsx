...
import { Send, MessageCircle, Brain, Sparkles, X } from 'lucide-react'
...
interface RenataCompactProps {
  isOpen: boolean
  onClose?: () => void
}

const RenataCompact: React.FC<RenataCompactProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className="flex flex-col bg-studio-bg border border-studio-border rounded-lg overflow-hidden relative"
      style={{ height: '300px' }}
    >
      {/* Header */}
      {onClose && (
        <div className="flex items-center justify-between p-2 border-b border-studio-border bg-studio-surface/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-studio-gold">Renata AI</span>
          </div>
          <button
            onClick={onClose}
            className="p-1 hover:bg-studio-border/20 rounded transition-colors"
          >
