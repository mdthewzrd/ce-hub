...
export default function Home() {
  ...
  const handleRunScan = async () => {
    ...
    try {
      ...
      // Get the currently selected project from projects API
      const projectsResponse = await fetch('http://localhost:5665/api/projects');
      const projectsData = await projectsResponse.json();

      // Find the most recent Backside B Scanner project (highest ID = newest)
      // Fixed: Check for the actual project name "Enhanced Backside B Scanner"
      // Fixed: Access projects array correctly from frontend API response
      const projectsArray = projectsData.data?.data || projectsData.data || projectsData || [];
      const backsideProjects = projectsArray.filter((p: any) =>
        (p.name === 'Enhanced Backside B Scanner' || p.name.includes('Backside B')) && p.code) || [];
      let selectedProject = backsideProjects.length > 0
        ? backsideProjects.reduce((newest: any, current: any) =>
            parseInt(current.id) > parseInt(newest.id) ? current : newest)
        : projectsArray.find((p: any) => p.enhanced && p.code) ||
          projectsArray[0]; // Fallback to first project

      if (!selectedProject || !selectedProject.code) {
        console.error('❌ No valid project with scanner code found');
        setScanResults([]);
      ...
      console.log('  Using project ID:', projectId, selectedProject.name || 'Default');

      const response = await fetch(`http://localhost:5665/api/projects/${projectId}/execute`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        if (data.success && data.execution_id) {
          ...
          const actualScanResults = data.results || [];
          ...
          const convertedResults = actualScanResults.map((result: any) => ({
            ...
            result: 'win', // Backend found valid trading signals
...
if __name__ == "__main__":
  ...
  const handleDeleteProject = async (projectId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5665/api/projects?id=${projectId}`, {
        method: 'DELETE'
      });
                          ...
                          if (response.ok) {
                            ...
                            try {
                              ...
                              const newScan = {
                                ...
                                scanParams: {
                                  start_date: formattedCode.startDate || new Date().toISOString().split('T')[0],
                                  end_date: formattedCode.endDate || new Date().toISOString().split('T')[0]
                                },
                                resultCount: scanData.resultCount,
