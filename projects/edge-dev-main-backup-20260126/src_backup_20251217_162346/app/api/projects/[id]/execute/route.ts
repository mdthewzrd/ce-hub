...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      if (error instanceof Error && error.message && error.message.includes('429 Too Many Requests')) {
        return NextResponse.json(
          {
            success: false,
      ...
      return NextResponse.json(
        {
          success: false,
          error: `Backend request failed: ${error instanceof Error ? error.name : 'Unknown Error'}`,
