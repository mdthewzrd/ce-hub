...
export class PythonExecutorService {
  ...
  async executeScanner(request: ExecutionRequest): Promise<ExecutionResult> {
    ...
    try {
      ...
      // TODO: Fix method visibility issue
      const results: any[] = [];
      // const results = await (this as any).executePythonFile(scannerFilePath, request);
...
# Extract function name from user's code and create wrapper
import re

# Infrastructure functions to exclude (added by sophisticated formatter)
infrastructure_functions = [
    ...
    'dates_before_after',
    'get_offsets',
    'run_enhanced_lc_scan'  # Add this too
]

print(f"🔧 Infrastructure functions to exclude: {infrastructure_functions}")

# Find ALL function definitions and filter out infrastructure functions
all_function_matches = re.findall(r'def\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(', '''${userCode}''')
...
for func_name in all_function_matches:
    if func_name not in infrastructure_functions:
        ...
        print(f"✅ Selected user function: {func_name}")
        break  # Use the first non-infrastructure function
    else:
        print(f"❌ Skipping infrastructure function: {func_name}")

if user_function_name:
    print(f"🎯 Final user function: {user_function_name}")
else:
    user_function_name = 'scan_symbol'
    print("⚠️ No user function found, defaulting to 'scan_symbol'")

# User's scanner code - MARKET-WIDE data is available as MARKET_WIDE_DATA
${userCode}

# Create a standard scan_symbol wrapper that calls the user's function
def scan_symbol(symbol, start_date, end_date):
    \"\"\"Standard wrapper that calls the user's actual function\"\"\"
    try:
        if user_function_name in globals():
            user_func = globals()[user_function_name]

            # Check function signature and adapt accordingly
            import inspect

            sig = inspect.signature(user_func)
            params = list(sig.parameters.keys())

            if len(params) >= 2 and 'symbol' in params and 'data' in params:
                # Function expects (symbol, data) - provide market data
                df_sym = MARKET_WIDE_DATA[MARKET_WIDE_DATA['ticker'] == symbol].copy()
                if not df_sym.empty:
                    ...
                    return user_func(symbol, df_sym)
            elif len(params) >= 2 and 'symbol' in params:
                # Function expects (symbol, start_date, end_date) or similar
                if len(params) == 2:
                    return user_func(symbol, start_date)
                else:
                    return user_func(symbol, start_date, end_date)
            else:
                # Function expects just symbol
                return user_func(symbol)

        else:
            print(f"❌ Function {user_function_name} not found in globals")
            return None

    except Exception as e:
        print(f"❌ Error in scan_symbol wrapper: {e}")
        return None
...
if __name__ == "__main__":
    try:
        ...
        if 'scan_symbol' in globals():
            ...
            for i in range(0, len(all_tickers), batch_size):
                ...
                print(f"📊 Batch {i//batch_size + 1}: Processing {len(batch_tickers)} tickers")
   ...
   */
  public async executePythonFile(filePath: string, request: ExecutionRequest): Promise<any[]> {
