...
// Configuration - Prioritize Next.js backend with persistent project storage
const FALLBACK_URLS = [
...
const BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5665'; // Primary server - Next.js with persistent storage
...
class ProjectApiService {
    ...
    for (const baseUrl of allUrls) {
      ...
      try {
        ...
        if (data && typeof data === 'object' && 'success' in data) {
          // For DELETE requests, don't try to transform project data
          ...
          if (data.success && data.data) {
            ...
            // Fallback if success field is false but data exists
            return this.transformProjectData(data.data) as T;
          }
        }
        ...
        // Return data as-is if it doesn't match expected format
        return data;
  ...
  async deleteProject(projectId: string): Promise<void> {
    await this.request<any>(`/projects/${projectId}`, {
      method: 'DELETE',
    });
    // We don't need to return the response data, just ensure the request succeeds
  }
