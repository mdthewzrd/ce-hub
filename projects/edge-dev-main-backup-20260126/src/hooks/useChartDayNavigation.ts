...
export function useChartDayNavigation({
  ...
  useEffect(() => {
    if (referenceDate) {
      ...
      // Calculate max available days (ensure at least 14 for +14D to work)
      const maxDays = calculateMaxAvailableDays(newState.referenceDay, 30);
      ...
      try {
        ...
        while (currentOffset !== offset) {
          if (currentOffset < offset) {
            ...
            targetDate = getPreviousTradingDay(targetDate);
            currentOffset--;
          }
        }
