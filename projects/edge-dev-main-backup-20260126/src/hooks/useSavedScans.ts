...
export function useSavedScans({
  ...
  const ensureArray = (data: any): SavedScan[] => {
    ...
    console.warn('savedScans was not an array, resetting to empty array:', data);
    return [];
  };
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success) {
            const backendScans = ensureArray(data.scans || []).map((scan: any) => ({
              id: scan.scan_id,
              name: scan.scan_name,
      ...
      const scans = ensureArray(loadSavedScans());
      setSavedScans(scans);
  ...
  const handleSaveScan = useCallback(async (scanData: Omit<SavedScan, 'id' | 'createdAt'>) => {
    try {
      ...
      setSavedScans(prev => [newScan, ...ensureArray(prev).filter(s => s.id !== newScan.id)]);
  ...
  const handleDeleteScan = useCallback(async (scanId: string) => {
    try {
      ...
      if (success) {
        setSavedScans(prev => ensureArray(prev).filter(scan => scan.id !== scanId));
        onScanDeleted?.(scanId);
  ...
  // Derived state
  const scansArray = ensureArray(savedScans);
  const totalScans = scansArray.length;
  const favoriteScans = scansArray.filter(scan => scan.isFavorite).length;
  const recentScans = scansArray
