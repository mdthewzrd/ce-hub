...
export const SavedScansSidebar: React.FC<SavedScansSidebarProps> = ({
  ...
  const filteredScans = (Array.isArray(savedScans) ? savedScans : []).filter(scan => {
    ...
    <div className="w-80 bg-gray-900 border-r border-gray-700 flex flex-col h-full">
      ...
      <div className="p-4 border-b border-gray-700">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="text-xs text-gray-400">
            {filteredScans.length} of {Array.isArray(savedScans) ? savedScans.length : 0}
          </div>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-gray-700">
        <div className="text-xs text-gray-500 text-center">
          {Array.isArray(savedScans) ? savedScans.length : 0} saved scan{(Array.isArray(savedScans) ? savedScans.length : 0) !== 1 ? 's' : ''}
          {Array.isArray(savedScans) && savedScans.filter(s => s.isFavorite).length > 0 && (
            <>
              {' • '}
