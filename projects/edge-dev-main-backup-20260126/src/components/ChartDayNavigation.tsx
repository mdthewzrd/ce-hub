...
export const ChartDayNavigation: React.FC<ChartDayNavigationProps> = ({
  ...
  const handleGoToDay = (targetDayOffset: number) => {
    if (!disabled && !isLoading && targetDayOffset <= maxAvailableDays && targetDayOffset >= -5) {
      onNavigate({ type: 'GO_TO_DAY', dayOffset: targetDayOffset });
    }
  };
    ...
    <div className="bg-gray-900 border border-gray-700 rounded-lg p-2 sm:p-3 lg:p-4 mb-4">
      ...
      <div className="mb-3">
        ...
        <div className="flex flex-wrap gap-1">
          {/* Key days that are most useful for LC pattern analysis */}
          <button
            onClick={() => handleGoToDay(0)}
            disabled={disabled || isLoading || dayOffset === 0}
            ...
            title="Go to LC Pattern Day (Day 0)"
          >
            D0
          </button>

          <button
            onClick={() => handleGoToDay(1)}
            disabled={disabled || isLoading || 1 > maxAvailableDays}
            className={`px-2 py-1 text-xs font-medium rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              dayOffset === 1
          ...
          <button
            onClick={() => handleGoToDay(3)}
            disabled={disabled || isLoading || 3 > maxAvailableDays}
            className={`px-2 py-1 text-xs font-medium rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              dayOffset === 3
                ? 'bg-green-500 text-white'
                : 3 <= maxAvailableDays
          ...
          <button
            onClick={() => handleGoToDay(7)}
            disabled={disabled || isLoading || 7 > maxAvailableDays}
            className={`px-2 py-1 text-xs font-medium rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              dayOffset === 7
                ? 'bg-green-500 text-white'
                : 7 <= maxAvailableDays
            ...
            title="Go to Day +7 (1 week)"
          >
            +7D
          </button>

          <button
            onClick={() => handleGoToDay(14)}
            disabled={disabled || isLoading || 14 > maxAvailableDays}
            className={`px-2 py-1 text-xs font-medium rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              dayOffset === 14
            ...
            title="Go to Day +14 (2 weeks)"
          >
            +14D
          </button>

          <button
            onClick={() => handleGoToDay(21)}
            disabled={disabled || isLoading || 21 > maxAvailableDays}
            className={`px-2 py-1 text-xs font-medium rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              dayOffset === 21
