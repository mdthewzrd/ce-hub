...
export const calculateGlobalDataBounds = (
  ...
  if (timeframe === 'day') {
    ...
    if (dayOffset === 0 && dayNavigation?.referenceDay) {
      ...
      const referenceDayStr = dayNavigation.referenceDay.toLocaleDateString('en-US', { timeZone: 'America/New_York' });
      // 🔧 CRITICAL FIX: Set market close at 4pm ET exactly
      const marketCloseET = new Date(dayNavigation.referenceDay);
      marketCloseET.setHours(16, 0, 0, 0); // 4:00:00 PM ET
      xRange = [data.x[0], marketCloseET.toLocaleString('en-US', { timeZone: 'America/New_York' })];
    } else {
