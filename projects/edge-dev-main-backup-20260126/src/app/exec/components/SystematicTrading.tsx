...
const SystematicTrading: React.FC<SystematicTradingProps> = ({ isVisible, onClose, onScanComplete, onStrategyUploaded, uploadedScannerCode }) => {
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      // Request browser notification permission - DISABLED
  ...
  const saveScanResults = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log('✅ Save successful - notifications disabled');
  ...
  const loadUserScans = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5666/api/scans/user/${userId}`);
      const data = await response.json();
  ...
  const loadScan = async (scanId: string) => {
    try {
      const response = await fetch('http://localhost:5666/api/scans/load', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        ...
        // Show browser notification - DISABLED
        console.log('✅ Load successful - notifications disabled');
