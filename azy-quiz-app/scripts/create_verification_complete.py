...
""")
        ...
        for handle, prod in list(products.items())[:150]:  # Limit to 150 for performance
            ...
            # Build face shapes HTML
            face_shapes_html = ''
            if existing_face_shapes and any(s.strip() for s in existing_face_shapes):
                for s in existing_face_shapes[:5]:
                    if s.strip():
                        face_shapes_html += f'<span class="selected-tag face-shape">{s.strip()}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'face_shapes\\', \\'{s.strip()}\\')">×</button></span>'
            else:
                face_shapes_html = '<span class="no-selection">Not tagged</span>'
            ...
            if existing_use_cases and any(s.strip() for s in existing_use_cases):
                for s in existing_use_cases[:6]:
                    if s.strip():
                        ...
                        use_cases_html += f'<span class="selected-tag use-case">{label}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'use_cases\\', \\'{s.strip()}\\')">×</button></span>'
            else:
                use_cases_html = '<span class="no-selection">Not tagged</span>'

            # Build lens types HTML
            lens_types_html = ''
            if existing_lens_types and any(s.strip() for s in existing_lens_types):
                for s in existing_lens_types[:5]:
                    if s.strip():
                        lens_types_html += f'<span class="selected-tag lens">{s.strip()}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'lens_types\\', \\'{s.strip()}\\')">×</button></span>'
            else:
                lens_types_html = '<span class="no-selection">Not tagged</span>'

            # Build style selection HTML
            if existing_style:
                style_html = f'<span class="selected-tag style">{existing_style}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'style\\', \\'{existing_style}\\')">×</button></span>'
            else:
                style_html = '<span class="no-selection">Not tagged</span>'

            # Build material selection HTML
            if existing_material:
                material_html = f'<span class="selected-tag material">{existing_material}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'material\\', \\'{existing_material}\\')">×</button></span>'
            else:
                material_html = '<span class="no-selection">Not tagged</span>'
            ...
            if (category === 'face_shapes' || category === 'use_cases' || category === 'lens_types') {
                ...
                if (value && value.length > 0) {
                    selectionDiv.innerHTML = value.map(v =>
                        `<span class="selected-tag ${category === 'face_shapes' ? 'face-shape' : ''}">${v.replace(/_/g, ' ')}<button class="remove-tag" onclick="removeTag('${handle}', '${category}', '${v}')">×</button></span>`
                    ).join('');
                } else {
