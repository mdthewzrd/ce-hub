...
Pre-populates with existing tags for easy verification
"""

import csv

def read_products():
    """Read tagged products with images"""
    products = {}
    seen = set()
...
def create_comprehensive_html(products, tags):
    """Create comprehensive review page with correct/wrong/edit"""
...
<body>
    <div class="header">
        ...
        <p>Review existing tags: ✓ Correct | ✗ Wrong | ✏️ Edit</p>
    </div>
    ...
    <div class="filters">
        ...
        <div class="filter-buttons">
            ...
            <button class="filter-btn" onclick="filterProducts('untagged')">Untagged</button>
        </div>
    </div>
...
""")
        ...
        for handle, prod in products.items():
            ...
            face_shapes_array = str([f"'{s.strip()}'" for s in existing_face_shapes if s.strip()])
            use_cases_array = str([f"'{s.strip()}'" for s in existing_use_cases if s.strip()])
            lens_types_array = str([f"'{s.strip()}'" for s in existing_lens_types if s.strip()])

            f.write(f"""        <div class="product-card" data-handle="{handle}" data-reviewed="no" data-tagged="{str(bool(existing_style)).lower()}">
            ...
            <div class="product-info">
                ...
                <div class="tag-section">
                    ...
                    <div class="current-selection" id="style-selection-{handle}">
                        {f'<span class="selected-tag style">{existing_style}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'style\\')">×</button></span>' if existing_style else '<span class="no-selection">Not tagged</span>'}
                    </div>
                    <div class="verification-buttons">
                        <button class="verify-btn correct" onclick="verifyTag('{handle}', 'style', '{existing_style}')">✓ Correct</button>
                ...
                <div class="tag-section">
                    ...
                    <div class="current-selection" id="material-selection-{handle}">
                        {f'<span class="selected-tag material">{existing_style}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'material\\')">×</button></span>' if existing_material else '<span class="no-selection">Not tagged</span>'}
                    </div>
                    <div class="verification-buttons">
                        <button class="verify-btn correct" onclick="verifyTag('{handle}', 'material', '{existing_material}')">✓ Correct</button>
                ...
                <div class="tag-section">
                    ...
                    <div class="current-selection" id="face_shapes-selection-{handle}">
                        {f'<span class="selected-tag face-shape">{s}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'face_shapes\\', \\'{s}\\')">×</button></span>' for s in existing_face_shapes if s.strip() if existing_face_shapes else ['<span class="no-selection">Not tagged</span>'][0:5]}
                    </div>
                    <div class="verification-buttons">
                        <button class="verify-btn correct" onclick="verifyTagMultiple('{handle}', 'face_shapes', {face_shapes_array})">✓ Correct</button>
                ...
                <div class="tag-section">
                    ...
                    <div class="current-selection" id="use_cases-selection-{handle}">
                        {f'<span class="selected-tag use-case">{s.replace("_", " ").title()}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'use_cases\\', \\'{s}\\')">×</button></span>' for s in existing_use_cases if s.strip() if existing_use_cases else ['<span class="no-selection">Not tagged</span>'][0:6]}
                    </div>
                    <div class="verification-buttons">
                        <button class="verify-btn correct" onclick="verifyTagMultiple('{handle}', 'use_cases', {use_cases_array})">✓ Correct</button>
                ...
                <div class="tag-section">
                    ...
                    <div class="current-selection" id="lens_types-selection-{handle}">
                        {f'<span class="selected-tag lens">{s}<button class="remove-tag" onclick="removeTag(\\'{handle}\\', \\'lens_types\\', \\'{s}\\')">×</button></span>' for s in existing_lens_types if s.strip() if existing_lens_types else ['<span class="no-selection">Not tagged</span>'][0:5]}
                    </div>
                    <div class="verification-buttons">
                        <button class="verify-btn correct" onclick="verifyTagMultiple('{handle}', 'lens_types', {lens_types_array})">✓ Correct</button>
...
""")
    ...
    <script>
        ...
        function verifyTagMultiple(handle, category, values) {
            setStatus(handle, category, 'verified');
            productTags[handle] = productTags[handle] || {};
            productTags[handle][category] = values;
            console.log(`${handle} ${category}: ✓ ${values.join(', ')}`);
        }
            ...
            if (category === 'face_shapes' || category === 'use_cases' || category === 'lens_types') {
                ...
                if (value && value.length > 0) {
                    selectionDiv.innerHTML = value.map(v =>
                        `<span class="selected-tag ${category === 'face_shapes' ? 'face-shape' : category}">${v.replace(/_/g, ' ')}<button class="remove-tag" onclick="removeTag('${handle}', '${category}', '${v}')">×</button></span>`
                    ).join('');
                } else {
                ...
                // Single select
                if (value) {
                    selectionDiv.innerHTML = `<span class="selected-tag ${category}">${value}<button class="remove-tag" onclick="removeTag('${handle}', '${category}', '${value}')">×</button></span>`;
                } else {
                    selectionDiv.innerHTML = '<span class="no-selection">Not tagged</span>';
                }
            }
        ...
        function filterProducts(filter) {
            ...
            document.querySelectorAll('.product-card').forEach(card => {
                ...
                const tagged = card.getAttribute('data-tagged') === 'true';
                ...
                if (filter === 'all') {
                    ...
                    card.classList.toggle('hidden', tagged);
                }
            });
        }
