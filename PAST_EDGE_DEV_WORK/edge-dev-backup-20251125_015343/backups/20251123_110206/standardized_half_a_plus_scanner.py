# Standardized Half A+ Scanner
# Compatible with Universal Scanner Engine
# Original: /Users/michaeldurante/.anaconda/working code/Daily Para/half A+ scan.py
...
# ───────── Original Metric Computation Functions ─────────
...
def scan_symbol(symbol: str, start_date: str, end_date: str) -> List[Dict[str, Any]]:
    ...
    Compatible with Universal Scanner Engine
    Maintains original Half A+ Daily Para logic
    """
    try:
        ...
        # Apply original scan logic
        hits_df = scan_daily_para(df, custom_params)
        if hits_df.empty:
            return []

        results = []

        # Convert DataFrame results to standardized format
        for date, row in hits_df.iterrows():
            ...
            result = {
                ...
                'volume_ratio': round(float(row.get('Vol_over_AVG', 0)), 2),
                'signal_strength': 'Strong' if row.get('Gap_over_ATR', 0) > 1.0 else 'Moderate',
                'entry_price': round(float(row.get('Open', 0)), 2),
                'target_price': round(float(row.get('Open', 0) * 1.08), 2),  # 8% target

                # Original detailed metrics (preserved)
                'atr_multiple': round(float(row.get('Range_over_ATR', 0)), 2),
                'volume_multiple': round(float(row.get('Vol_over_AVG', 0)), 2),
                ...
                'high_ema9_atr': round(float(row.get('High_over_EMA9_div_ATR', 0)), 2),
                'high_ema20_atr': round(float(row.get('High_over_EMA20_div_ATR', 0)), 2),
                'pct_7d_low_atr': round(float(row.get('Pct_7d_low_div_ATR', 0)), 2),
                'pct_14d_low_atr': round(float(row.get('Pct_14d_low_div_ATR', 0)), 2),
                'gap_atr': round(float(row.get('Gap_over_ATR', 0)), 2),
                'open_ema9_ratio': round(float(row.get('Open_over_EMA9', 0)), 2),
                'atr_pct_change': round(float(row.get('ATR_Pct_Change', 0)), 2),
                'prev_close': round(float(row.get('Prev_Close', 0)), 2),
                'move_2d_atr': round(float(row.get('Move2d_div_ATR', 0)), 2),
                'move_3d_atr': round(float(row.get('Move3d_div_ATR', 0)), 2),
                'prev_gain_pct': round(float(row.get('Prev_Gain_Pct', 0)), 2),
                'open_above_prev_high': bool(row.get('Open', 0) > row.get('Prev_High', 0)),
            }
...
SCANNER_CONFIG = {
    'name': 'Standardized Half A+ Scanner',
    'description': 'Daily para optimized scan with session reuse & parallel execution - standardized',
    'timeframe': 'Daily',
    'original_path': '/Users/michaeldurante/.anaconda/working code/Daily Para/half A+ scan.py',
    'standardized': True,
...
if __name__ == "__main__":
    print("🎯 Standardized Half A+ Scanner - Test Mode")
