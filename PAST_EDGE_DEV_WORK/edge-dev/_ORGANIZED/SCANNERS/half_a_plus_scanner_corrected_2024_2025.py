#!/usr/bin/env python3
"""
Half A+ Scanner - CORRECTED for 1/1/24 - 11/1/25
===============================================
Fixed parameter integrity - should produce 8-15 signals only
"""
...
# ───────── CORRECTED HALF A+ PARAMETERS (ORIGINAL VALUES) ─────────
P = {
    # hard liquidity / price (BACK TO ORIGINAL)
    "price_min": 8.0,               # Restored from 5.0
    "adv20_min_usd": 30_000_000,     # Restored from 10M

    # backside context (BACK TO ORIGINAL)
    "abs_lookback_days": 1000,       # Restored from 800
    "abs_exclude_days": 10,          # Restored from 7
    "pos_abs_max": 0.75,             # Restored from 0.80

    # trigger mold (BACK TO ORIGINAL)
    "trigger_mode": "D1_or_D2",      # Keep this
    "atr_mult": 0.9,                 # Restored from 0.7
    "vol_mult": 0.9,                 # Restored from 0.7

    # Absolute D-1 volume floor (BACK TO ORIGINAL)
    "d1_volume_min": 15_000_000,     # Restored from 8M
    "d1_vol_mult_min": None,         # Keep disabled

    # momentum parameters (BACK TO ORIGINAL)
    "slope5d_min": 3.0,              # Restored from 2.0
    "high_ema9_mult": 1.05,          # Restored from 1.03

    # trade-day (D0) gates (BACK TO ORIGINAL)
    "gap_div_atr_min": 0.75,         # Restored from 0.5
    "open_over_ema9_min": 0.9,       # Restored from 0.85
    "d1_green_atr_min": 0.30,        # Restored from 0.20
    "require_open_gt_prev_high": True, # Restored from False

    # relative requirement
    "enforce_d1_above_d2": True,     # Keep this for quality
}

# ───────── ORIGINAL SYMBOL UNIVERSE ─────────
SYMBOLS = [
...
def _mold_on_row(rx: pd.Series) -> bool:
    """Check if row meets ORIGINAL mold criteria"""
...
def scan_symbol(sym: str, start: str, end: str) -> pd.DataFrame:
    """Scan single symbol for Half A+ patterns with CORRECTED parameters"""
    try:
        ...
        for i in range(2, len(m)):
            ...
            # Backside context check (ORIGINAL)
            lo_abs, hi_abs = abs_top_window(m, d0, P["abs_lookback_days"], P["abs_exclude_days"])
            pos_abs_prev = pos_between(r1["Close"], lo_abs, hi_abs)
            if not (pd.notna(pos_abs_prev) and pos_abs_prev <= P["pos_abs_max"]):
                continue

            # Trigger validation (ORIGINAL)
            trigger_ok = False
            trig_row = None
            trig_tag = "-"
            ...
            # D-1 must be green (ORIGINAL)
            if not (pd.notna(r1["Body_over_ATR"]) and r1["Body_over_ATR"] >= P["d1_green_atr_min"]):
                continue

            # Volume requirements (ORIGINAL)
            ...
            # D-1 > D-2 requirement (ORIGINAL)
            if P["enforce_d1_above_d2"]:
                if not (pd.notna(r1["High"]) and pd.notna(r2["High"]) and r1["High"] > r2["High"]
                        and pd.notna(r1["Close"]) and pd.notna(r2["Close"]) and r1["Close"] > r2["Close"]):
                    continue

            # D0 entry gates (ORIGINAL)
            ...
            rows.append({
                ...
                "Scanner_Type": "Half_A+_CORRECTED",
            })
...
# ───────── MAIN EXECUTION ─────────
def main():
    print("🎯 HALF A+ SCANNER - CORRECTED PARAMETERS")
    print("🔧 RESTORED: Should produce 8-15 signals only")
    print("=" * 60)
    ...
    if results:
        ...
        print(f"\n🎯 HALF A+ CORRECTED RESULTS: {len(out)} signals (EXPECTED: 8-15)")
        print("=" * 120)
        print(out.to_string(index=False))

        # Save corrected results
        output_file = "half_a_plus_corrected_2024_2025.csv"
