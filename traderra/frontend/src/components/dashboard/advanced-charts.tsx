...
const validateDataConsistency = <T extends { date: string }>(data: T[], minRequired: number = 5): T[] => {
  ...
  console.log(`Data validation: ${validData.length}/${data.length} valid entries (min required: ${minRequired})`)
...
const CustomTooltip = ({ active, payload, label }: any) => {
  const { displayMode } = useDateRange()
...
export function AdvancedEquityChart() {
  const { getFilteredData, getDateRangeLabel, displayMode } = useDateRange()
  const filteredData = getFilteredData(equityData)
...
export function PerformanceDistributionChart() {
  ...
  // Normalize all date formats to YYYY-MM-DD and validate data
  const normalizedData = dailyPnLDistribution.map(item => ({
    ...item,
    date: normalizeDate(item.date)
  }))

  // Validate and filter data
  const validatedData = validateDataConsistency(normalizedData, 3)
  const filteredData = validatedData.length > 0 ? getFilteredData(validatedData) : []
...
export function SymbolPerformanceChart() {
  const { getDateRangeLabel, displayMode, getFilteredData } = useDateRange()
  ...
  // Validate and filter trades by date range
  const validatedTrades = validateDataConsistency(symbolTradesData, 1)
  const filteredTrades = validatedTrades.length > 0 ? getFilteredData(validatedTrades) : []

  // Ensure we have sufficient data to display
  if (filteredTrades.length === 0) {
    console.warn('No valid symbol trades data available for the selected date range')
  }

  // Aggregate P&L by symbol from filtered trades
  const symbolPerformance = filteredTrades.reduce((acc: { [key: string]: { symbol: string; pnl: number; trades: number } }, trade) => {
    if (!acc[trade.symbol]) {
      acc[trade.symbol] = { symbol: trade.symbol, pnl: 0, trades: 0 }
    }
    acc[trade.symbol].pnl += trade.pnl
    acc[trade.symbol].trades += trade.trades
    return acc
  }, {})

  // Convert to array and sort by PnL for better visualization (highest to lowest)
  const sortedData = Object.values(symbolPerformance).sort((a, b) => b.pnl - a.pnl)

  // Ensure minimum display count for better UX
  const displayData = sortedData.length < 3 && symbolTradesData.length > 10
    ? Object.values(symbolTradesData.reduce((acc: { [key: string]: { symbol: string; pnl: number; trades: number } }, trade) => {
        if (!acc[trade.symbol]) {
          acc[trade.symbol] = { symbol: trade.symbol, pnl: 0, trades: 0 }
        }
        acc[trade.symbol].pnl += trade.pnl
    ...
    <div className="chart-container" data-testid="symbol-performance">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-sm studio-muted">
          {displayData.length} symbols {filteredTrades.length !== symbolTradesData.length && '(filtered)'}
        </div>
      </div>
      ...
      {displayData.length === 0 ? (
        ...
        <div className="h-80 overflow-y-auto space-y-2 hover:scrollbar-thin hover:scrollbar-track-[#1a1a1a] hover:scrollbar-thumb-[#333333]">
          {displayData.map((entry, index) => {
            ...
            const maxAbsValue = Math.max(...displayData.map(d => Math.abs(d.pnl)))
            const barWidth = maxAbsValue > 0 ? Math.abs(entry.pnl) / maxAbsValue * 100 : 0
...
export function BiggestTradesChart() {
  const [showWins, setShowWins] = useState(true)
  const { getDateRangeLabel, getFilteredData } = useDateRange()

  // Validate and filter trades based on date range
  const validatedBestTrades = validateDataConsistency(bestTradesData, 1)
  const validatedWorstTrades = validateDataConsistency(worstTradesData, 1)

  const filteredBestTrades = validatedBestTrades.length > 0 ? getFilteredData(validatedBestTrades) : []
  const filteredWorstTrades = validatedWorstTrades.length > 0 ? getFilteredData(validatedWorstTrades) : []

  // Fallback to unfiltered data if filtered data is insufficient (less than 3 trades)
  const fallbackBestTrades = filteredBestTrades.length < 3 ? validatedBestTrades.slice(0, 10) : filteredBestTrades
  const fallbackWorstTrades = filteredWorstTrades.length < 3 ? validatedWorstTrades.slice(0, 10) : filteredWorstTrades

  const currentData = showWins ? fallbackBestTrades : fallbackWorstTrades
  const isFiltered = showWins
    ? filteredBestTrades.length !== validatedBestTrades.length
    : filteredWorstTrades.length !== validatedWorstTrades.length
    ...
    <div className="chart-container" data-testid="best-trades">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-4">
          <div className="text-sm studio-muted">
            {currentData.length} trades {isFiltered && '(filtered)'}
          </div>
        <div className="flex items-center space-x-2">
          <button
